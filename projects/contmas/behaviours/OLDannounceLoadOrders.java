...
public class OLDannounceLoadOrders extends ContractNetInitiator{
	/**
	 * 
	 */
	...
	public OLDannounceLoadOrders(Agent a,TransportOrderChain currentTOC){
		super(a,null);
	...
	protected Vector<ACLMessage> prepareCfps(ACLMessage cfp){
		...
		if(oldState instanceof Announced){
			...
			if((contractorList == null) || contractorList.isEmpty()){
				notifyMaster("MasterBehaviour wird neugestartet.");
	...
	class handleAllResultNotifications extends FSMBehaviour{
		...
		handleAllResultNotifications(Agent a,DataStore ds){
			...
			*/
			...
			registerLastState(new DummyState(myAgent,getDataStore()),DUMMY);

			//register transitions
			registerTransition(START_MOVING,WAIT_UNTIL_TARGET_REACHED,CONTRACTOR_READY);
			registerTransition(START_MOVING,DUMMY,CONTRACTOR_NOT_READY);
