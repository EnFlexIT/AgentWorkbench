...
public class BasicGraphGuiTools implements ActionListener {
    ...
    /**
     * Get the component types definition dialog. New dialog object is created each time.
     * 
     * @return
     */
	...
	if (ae.getSource() == getJButtonComponents()) {
		...
		if (this.controller.getNetworkModel().isFreeNode(node1) && this.controller.getNetworkModel().isFreeNode(node2)) {
		    ...
		    JOptionPane.showMessageDialog(graphEnvGUI, Language.translate("Select two valid vertices", Language.EN), Language.translate("Warning", Language.EN), JOptionPane.WARNING_MESSAGE);
		}
	    } else {
		// --- Two nodes are not picked
		JOptionPane
			.showMessageDialog(graphEnvGUI, Language.translate("Use Shift and click two vertices", Language.EN), Language.translate("Warning", Language.EN), JOptionPane.WARNING_MESSAGE);
	    }
		...
		if (this.controller.getNetworkModel().getNetworkComponentCount(pickedVertex) == 2) {
		    ...
		    JOptionPane.showMessageDialog(graphEnvGUI, Language.translate("The select Vertex should be between two components !", Language.EN), Language.translate("Warning", Language.EN),
	    ...
	    // ------------------------------------------------------
	    // --- Button Clear graph -------------------------------
	    int n = JOptionPane.showConfirmDialog(Application.MainWindow, Language.translate("Are you sure that you want to clear the graph?", Language.EN),
		    Language.translate("Confirmation", Language.EN), JOptionPane.YES_NO_OPTION);
	    ...
	    graphFC.setFileFilter(new FileNameExtensionFilter(Language.translate(this.controller.getGraphFileImporter().getTypeString()), this.controller.getGraphFileImporter()
		    .getGraphFileExtension()));
