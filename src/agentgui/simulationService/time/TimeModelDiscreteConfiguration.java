 ...
 */
public class TimeModelDiscreteConfiguration extends DisplayJPanel4Configuration implements ActionListener, ChangeListener {
		...
		if (jComboBoxWidthUnit == null) {
			...
			jComboBoxWidthUnit.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					System.out.println("Changed Value of unit");
				}
			}); 
		}
	...
	public void setTimeModel(TimeModel timeModel) {
		...
		TimeModelDiscrete tmd = null;
		if (timeModel==null) {
			tmd = new TimeModelDiscrete();
		} else {
			tmd = (TimeModelDiscrete) timeModel;
		}
		
		// --- Start settings ---------------------------------------
		Date startDate = new Date(tmd.getTimeStart());
		...
		// --- Stop settings ----------------------------------------
		Date stopDate = new Date(tmd.getTimeStop());
		...
		long step = tmd.getStep();
		int unitSelection = tmd.getStepDisplayUnitAsIndexOfTimeUnitVector();
	...
	public TimeModel getTimeModel() {
		...
		// --- Set TimeModel ----------------------------------------
		TimeModelDiscrete tmd = new TimeModelDiscrete(startLong, stopLong, step);
		tmd.setStepDisplayUnitAsIndexOfTimeUnitVector(indexSelected);
		return tmd;
	}
