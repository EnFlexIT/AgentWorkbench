 ...
 *
 */
public class Old_EnvironmentControllerGUI extends JSplitPane {
	...
	private Old_EnvironmentController ec = null;
	 ...
	 */
	public Old_EnvironmentControllerGUI(Project project) {
		super();
		this.currentProject = project;
		this.ec = new Old_EnvironmentController(this, currentProject);
	...
	private void initialize() {
		...
		this.addComponentListener(new ComponentAdapter(){
			public void componentResized(ComponentEvent ce){
				Old_EnvironmentControllerGUI.this.setDividerLocation(Old_EnvironmentControllerGUI.this.getWidth()-220);
				splitControlls.setDividerLocation(Old_EnvironmentControllerGUI.this.getHeight()-250);				
			}
		});
	}
		...
		if (btnOpen == null) {
			...
			btnOpen.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent arg0) {
					if(Old_EnvironmentControllerGUI.this.getFcOpen().showOpenDialog(Old_EnvironmentControllerGUI.this)==JFileChooser.APPROVE_OPTION){
						ec.setNewEnv(true);
			...
			treeEnvironment.addTreeSelectionListener(new TreeSelectionListener(){
				...
				public void valueChanged(TreeSelectionEvent arg0) {
					if(treeEnvironment.getLastSelectedPathComponent()!=null){
						...
						Old_BasicObject object = ec.getMainPlayground().getObjects().get(treeEnvironment.getLastSelectedPathComponent().toString());
						if(object!=null){
		...
		if (btnApply == null) {
			...
			btnApply.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent e) {
					...
					Old_Playground pg = ec.getMainPlayground();
					...
					if(cbType.getSelectedItem().equals(Language.translate("Hindernis"))){
						Old_ObstacleObject newObject = new Old_ObstacleObject(id, selectedElement);
						...
						Old_AgentObject newAgent = new Old_AgentObject(id, selectedElement, agentClasses.get(cbClass.getSelectedItem().toString()));
		...
		if(canvas == null){
			...
			canvas.addSVGDocumentLoaderListener(new SVGDocumentLoaderAdapter(){
				...
				public void documentLoadingCompleted(SVGDocumentLoaderEvent e){
					...
					if(ec.isNewEnv()){
						ec.setMainPlayground  (new Old_Playground(canvas.getSVGDocument().getDocumentElement()));
					}
	 ...
	 */
	private DefaultMutableTreeNode buildSubTree(Old_Playground pg){
		String id = pg.getId();
		...
		if(pg!=null){
			...
			Iterator<Old_BasicObject> objects = pg.getObjects().values().iterator();
			while(objects.hasNext()){
				Old_BasicObject object = objects.next();
				if(object instanceof Old_Playground){
					playgroundsRoot.add(buildSubTree((Old_Playground) object));				
				}else if(object instanceof Old_AgentObject){
					agentsRoot.add(new DefaultMutableTreeNode(object.getId()));
				}else if(object instanceof Old_ObstacleObject){
	...
	private void setInputValues(){
		...
		if(selectedElement == null){
			...
			Old_BasicObject object = ec.getMainPlayground().getObjects().get(id);
			if(object != null){
				if(object instanceof Old_ObstacleObject){
					cbType.setSelectedItem(Language.translate("Hindernis"));
					cbClass.setEnabled(false);
					btnDelete.setEnabled(true);
				}else if(object instanceof Old_AgentObject){
					cbType.setSelectedItem(Language.translate("Agent"));
					cbClass.setSelectedItem(((Old_AgentObject) object).getAgentClass());
					cbClass.setEnabled(true);
					btnDelete.setEnabled(true);
				}else{
	...
	private JButton getBtnSave() {
		if (btnSave == null) {
			...
			btnSave.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent arg0) {
					File envFile = null;
					if(getFcSave().showSaveDialog(Old_EnvironmentControllerGUI.this) == JFileChooser.APPROVE_OPTION){
