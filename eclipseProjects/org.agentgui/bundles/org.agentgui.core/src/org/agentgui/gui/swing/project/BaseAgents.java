...
public class BaseAgents extends JSplitPane implements Observer, ActionListener {
	...
	if (Up2TreeLevel == null)
	    Up2TreeLevel = 1000;
	OntoTreeExpand(new TreePath(currProject.getOntologyVisualisationHelper().getOntologyTree().getRoot()), expand,
		CurrNodeLevel, Up2TreeLevel);
    }
	...
	if (jAgentList == null) {
		...
		public void valueChanged(ListSelectionEvent se) {
			...
			int i = 1;
			startAsNew = startAs;
			while (Application.getJadePlatform().isAgentRunning(startAs,
				currProject.getProjectFolder()) == true) {
	...
	if (trigger == jButtonStartAgent) {
		...
		Vector<AgentStartArgument> startArgsConfigured = this.currProject.getAgentStartConfiguration()
			.get(agentClassName);
		if (startArgsConfigured != null) {
		    // --- If start-arguments are set, get them now -----
		    OntologyInstanceDialog oid = new OntologyInstanceDialog(Application.getMainWindow(),
			    this.currProject.getOntologyVisualisationHelper(),
			    this.currProject.getAgentStartConfiguration(), agentClassName);
		    oid.setTitle(Language.translate("Startargument definieren für Agent") + " '" + agentName + "' ("
			    + agentClassNameSimple + ")");
		    oid.setVisible(true);
		    // --- Wait ---
		    if (oid.isCancelled())
			return;
		...
		// --- Start the Agent now --------------------------
		Application.getJadePlatform().startAgent(agentName, agentClassName, startArgs,
		...
		String msg = Language.translate(
			"Bitte wählen Sie einen Agenten und eine Ontologie-Referenz aus den entsprechenden Listen.");
		JOptionPane.showMessageDialog(Application.getMainWindow(), msg, head, JOptionPane.WARNING_MESSAGE);
		return;
	    }
	    ...
	    this.readSelectionFromForm();
	    int newPos = this.currProject.getAgentStartConfiguration().removeReference(this.agentReference,
		    this.getJListReferences().getSelectedIndex());
	    this.currProject.setAgentStartConfigurationUpdated();
	    if (newPos != -1) {
		this.getJListReferences().setSelectedIndex(newPos);
	    }
	    ...
	    this.readSelectionFromForm();
	    int newPos = this.currProject.getAgentStartConfiguration().movePosition(agentReference,
		    jListReferences.getSelectedIndex(), -1);
	    this.currProject.setAgentStartConfigurationUpdated();
	    if (newPos != -1) {
	    ...
	    AgentStartArgument ageStartArg = this.currProject.getAgentStartConfiguration().get(this.agentReference)
		    .get(selectedIndex);
	    String displayTitle = ageStartArg.getDisplayTitle();
	    if (displayTitle == null) {
		input = JOptionPane.showInputDialog(Application.getMainWindow(), msg, head,
			JOptionPane.QUESTION_MESSAGE);
	    } else {
		input = (String) JOptionPane.showInputDialog(Application.getMainWindow(), msg, head,
			JOptionPane.QUESTION_MESSAGE, null, null, displayTitle);
	    }
	    if (input == null) {
		return;
	    } else {
	...
	if (this.agentReference == null) {
	    ...
	    Vector<AgentStartArgument> arguments = this.currProject.getAgentStartConfiguration()
		    .get(this.agentReference);
	    if (arguments != null) {
