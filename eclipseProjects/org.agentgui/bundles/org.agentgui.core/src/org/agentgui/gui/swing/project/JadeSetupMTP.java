...
//	private boolean pauseDocumentListener;
	...
	jLabelPortExplain.setText(
		Language.translate("(Falls bereits verwendet, wird versucht den nächst höheren Port zu nutzen)"));
	...
	if (jCheckBoxSkipUserRequestForJadeStart == null) {
	    jCheckBoxSkipUserRequestForJadeStart = new JCheckBox(
		    Language.translate("JADE direkt starten ohne nachzufragen."));
	...
	if (jLabeSecurityPathInfo == null) {
	    jLabeSecurityPathInfo = new JLabel(
		    "(" + Language.translate("located in the projects 'security' directory", Language.EN) + ")");
	...
	if (jButtonDeleteSecurityStores == null) {
	    ...
	    jButtonDeleteSecurityStores
		    .setToolTipText(Language.translate("Delete security store settings", Language.EN));
	...
	this.getJCheckBoxSkipUserRequestForJadeStart()
		.setSelected(this.currProject.getJadeConfiguration().isSkipUserRequestForJadeStart());
	...
	if (trigger == this.getJCheckBoxSkipUserRequestForJadeStart()) {
	    ...
	    this.currProject.getJadeConfiguration()
		    .setSkipUserRequestForJadeStart(this.getJCheckBoxSkipUserRequestForJadeStart().isSelected());

	} else if (trigger == this.getJButtonSetPort()) {
	    // --- Open Dialog ------------------
	    JadeSetupNewPort newPort = new JadeSetupNewPort(Application.getMainWindow(), currProject.getProjectName(),
		    true, this.currProject.getJadeConfiguration().getLocalPort(),
		    jTextFieldDefaultPort.getLocationOnScreen());
	    ...
	    // --- Set MTP port ---------------------------
	    JadeSetupNewPort newPort = new JadeSetupNewPort(Application.getMainWindow(),
		    this.currProject.getProjectName(), true, this.currProject.getJadeConfiguration().getLocalPortMTP(),
		    this.getJTextFieldDefaultPortMTP().getLocationOnScreen());
	    newPort.setVisible(true);
	    // === Go ahead =====================
	    if (newPort.isCanceled() == false) {
	    ...
	    if (this.currProject.getJadeConfiguration().getKeyStoreFile() != null
		    || this.currProject.getJadeConfiguration().getTrustStoreFile() != null) {
		// --- Ask the suer to delete the current settings ------------
		String title = Language.translate("Delete MTP security references?", Language.EN);
		String message = Language.translate("Would you like to delete the KeyStore and TrustStore references?",
			Language.EN);
		int userAnswer = JOptionPane.showConfirmDialog(Application.getMainWindow(), message, title,
	...
	if (keyStoreFilePath == null || trustStoreFilePath == null || new File(keyStoreFilePath).exists() == false
		|| new File(trustStoreFilePath).exists() == false) {
	    ...
	    // --- Open the HttpsConfigWindow ----------------------------------
	    HttpsConfigWindow httpsConfigWindow = new HttpsConfigWindow(Application.getMainWindow(), keyStoreFile,
     ...
     *
     * @param configuredMtpProtocol the configured MTP protocol
     * @param httpsConfigWindow     the current instance of the
     *                              {@link HttpsConfigWindow}
     */
    private void setHttpsConfigWindowSettingsToProject(MtpProtocol configuredMtpProtocol,
	    HttpsConfigWindow httpsConfigWindow) {
	...
	this.currProject.getJadeConfiguration()
		.setTrustStoreFile(httpsConfigWindow.getTrustStoreFile().getAbsolutePath());
