...
public class JPanelEmbeddedSystemAgent extends AbstractJPanelForOptions implements ActionListener {
	...
	jLabelEmbeddedHeader = new JLabel(this.getGlobalInfo().getApplicationTitle() + " - "
		+ Language.translate("Dienst") + " / Embedded System Agent");
	...
	if (jRadioButtonExecuteAsServerService == null) {
	    ...
	    jRadioButtonExecuteAsServerService
		    .setText(Language.translate(jRadioButtonExecuteAsServerService.getText()));
	...
	if (this.getJComboBoxProjectSelector().getSelectedItem() == null
		|| this.getJComboBoxProjectSelector().getSelectedItem().equals("") == true) {
	    ...
	    if (this.getJRadioButtonExecuteAsService().isSelected() == false
		    && this.getJRadioButtonExecuteAsDeviceAgent().isSelected() == false) {
     ...
     * 
     * @see
     * de.enflexit.common.swing.options.AbstractJPanelForOptions#setGlobalData2Form(
     * )
     */
    @Override
     ...
     * 
     * @see
     * de.enflexit.common.swing.options.AbstractJPanelForOptions#setFromData2Global(
     * )
     */
    @Override
	...
	this.getGlobalInfo()
		.setDeviceServiceProjectFolder((String) this.getJComboBoxProjectSelector().getSelectedItem());
	...
	if (this.getSelectedExecutionMode() == ExecutionMode.DEVICE_SYSTEM) {
		...
		JOptionPane.showMessageDialog(this.optionDialog.getContentPane(), msgText, msgHead,
		...
		msgText = Language.translate(
			"Bitte legen Sie fest, ob Sie Agent.GUI als Service oder als Embedded System Agent ausführen möchten")
			+ "!";
		JOptionPane.showMessageDialog(this.optionDialog.getContentPane(), msgText, msgHead,
			JOptionPane.ERROR_MESSAGE);
		return true;
		...
		if (this.getJComboBoxSetupSelector().getSelectedItem() == null
			|| this.getJComboBoxSetupSelector().getSelectedItem().equals("") == true) {
