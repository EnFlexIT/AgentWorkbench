...
public class JPanelMTPConfig extends AbstractJPanelForOptions implements ActionListener, ItemListener {
     ...
     * 
     * @return {@link JComboBoxMtpProtocol}
     */
	...
	if (trigger == this.getJButtonEditMTP()) {
	    ...
	    HttpsConfigWindow httpsConfigWindow = new HttpsConfigWindow(this.optionDialog, getKeyStore(),
		    getKeyStorePassword(), getTrustStore(), getTrustStorePassword());
	...
	if (event.getSource() == this.getJComboBoxMtpProtocol()) {
		...
		if (event.getStateChange() == ItemEvent.SELECTED) {
			...
			// --- If the stores are not defined or do not exist, open the configuration
			// dialog ----------
			if (this.keyStore == null || this.trustStore == null || this.keyStore.exists() == false
				|| this.trustStore.exists() == false) {
     ...
     * 
     * @see
     * de.enflexit.common.swing.options.AbstractJPanelForOptions#setFormData2Global(
     * )
     */
    @Override
