...
public class HttpsConfigWindow extends JDialog implements ActionListener {
     ...
     */
    public HttpsConfigWindow(Window owner, File keystore, String keystorePassword, File truststore,
	...
	this.setTitle(Application.getGlobalInfo().getApplicationTitle() + ": "
		+ Language.translate("HTTPS Configuration", Language.EN));
	...
	if (this.getPreferredDirector() == null)
	    return true;
	if (filePathToCheck == null || filePathToCheck.isEmpty())
	    return false;
     ...
     * 
     * @return String keystorePassword
     */
     ...
     * 
     * @param keystorebuttonPressed state of keyStoreButton
     */
     ...
     * 
     * @param truststorebuttonPressed shows state of truststore button
     */
	...
	if (controller.isInitialized()) {
	    certificateProperties = this.getKeyStoreConfigPanel().getKeyStoreController()
	...
	this.getTrustStoreConfigPanel().getTrustStoreController().openTrustStore(getTrustStoreFile(),
		getTrustStorePassword());
	this.getTrustStoreConfigPanel().getTrustStoreController().getTrustedCertificatesList();
	this.getTrustStoreConfigPanel().getjTableTrusTedCertificates()
		.setModel(getTrustStoreConfigPanel().getTrustStoreController().getTableModel());
	...
	if (ae.getSource() == this.getJButtonCreateKeyStore()) {
		...
		if (this.getPreferredDirector() != null) {
			...
			String msg = Language.translate("The KeyStore file should be located in directory", Language.EN)
				+ " '" + this.getPreferredDirector() + "'!";
		...
		if (keyStoreName.endsWith(KEYSTORE_FILENAME)) {
		    ...
		    JPanelPassword jPanelEnterPassword = new JPanelPassword(
			    Language.translate("Please enter the password for  ", Language.EN) + " " + keyStoreName
				    + ":");
		    int option = JOptionPane.showOptionDialog(null, jPanelEnterPassword, title,
			    JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, null, null, null);
		    if (option == JOptionPane.OK_OPTION) {
			// ---- Pressed OK Button -----------------------------
			if (this.getKeyStoreConfigPanel().getKeyStoreController().openTrustStore(keyStoreFile,
				jPanelEnterPassword.getPassword()) == true) {
			    keyStorePassword = jPanelEnterPassword.getPassword();
			    this.setKeyAlias(this.getKeyStoreConfigPanel().getKeyStoreController()
				    .getFirstCertificateProperties().getAlias());
			}
		    ...
		    String msg = Language.translate(
			    "Please choose a KeyStore file! Your KeyStore name should ends with 'KeyStore'!",
			    Language.EN);
		...
		if (this.getPreferredDirector() != null) {
		    if (this.isInPreferredDirector(this.getJFileChooser().getSelectedFile()) == false) {
			String msg = Language.translate("The TrustStore file should be located in directory",
				Language.EN) + " '" + this.getPreferredDirector() + "'!";
		...
		if (truststorename.endsWith(TRUSTSTORE_FILENAME)) {
		    ...
		    String title = Language.translate("Password", Language.EN);
		    JPanelPassword jPanelEnterPassword = new JPanelPassword(
			    Language.translate("Please enter the password for  ", Language.EN) + " " + truststorename
				    + ":");
		    int option = JOptionPane.showOptionDialog(null, jPanelEnterPassword, title,
			    JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, null, null, null);
		    if (option == JOptionPane.OK_OPTION) {
			// ---- Pressed OK Button -----------------------------
			if (this.getTrustStoreConfigPanel().getTrustStoreController().openTrustStore(trustStoreFile,
				jPanelEnterPassword.getPassword()) == true) {
		    ...
		    String msg = Language.translate(
			    "Please, choose a TrustStore file! Your TrustStore name should ends with 'TrustStore'",
			    Language.EN);
		    String title = Language.translate("Warning message", Language.EN);
		    JOptionPane.showMessageDialog(this, msg, title, JOptionPane.WARNING_MESSAGE);
		}
		...
		String msg = Language.translate("The TrustStore file should be located in directory", Language.EN)
			+ " '" + this.getPreferredDirector() + "'!";
		String title = Language.translate("Wrong TrustStore location!", Language.EN);
		JOptionPane.showMessageDialog(this, msg, title, JOptionPane.WARNING_MESSAGE);
	    } else {
