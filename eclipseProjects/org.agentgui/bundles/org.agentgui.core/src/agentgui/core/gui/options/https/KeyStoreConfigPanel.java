...
/**
 * This JPanel allows the user to : 1- create new KeyStore and protect its
 * integrity with a password 2- update his KeyStore informations 3- generate
 * certificate from the KeyStore
 * 
 * @see HttpsConfigWindow
 * 
 *      * @author <a href="mailto:mohamedamine_jedidi@outlook.com">Mohamed Amine
 *      JEDIDI</a>
 * @version 1.0
 * @since 05-04-2016
 */
...
//			jPanelCertificate.add(getJTextFieldCertificateValidity(), gbc_jTextFieldCertificateValidity);
	...
	if (jButtonCertificate == null) {
	    ...
	    jButtonCertificate
		    .setToolTipText(Language.translate("Generate your own certificate from the KeyStore", Language.EN));
	...
	if (ae.getSource() == this.getJButtonApplyKeyStore()) {
		...
		if (this.getJTextFieldKeyStoreName().getText().isEmpty()
			|| this.getJTextFieldAlias().getText().isEmpty() || this.getJTextFieldCity().getText().isEmpty()
			|| this.getJTextFieldCountryCode().getText().isEmpty()
			|| this.getJTextFieldFullName().getText().isEmpty()
			|| this.getJTextFieldOrganization().getText().isEmpty()
			|| this.getJTextFieldOrganizationalUnit().getText().isEmpty()
			|| this.getJTextFieldState().getText().isEmpty()
			|| this.getJTextFieldValidity().getText().isEmpty()
			|| this.getJPasswordField().getPassword().length == 0
			|| this.getJPasswordConfirmPassword().getPassword().length == 0) {
		...
		/*
		 * -------------------------------------------------------- ----- Verify if The
		 * password and its confirm ----------- ---------------- are the same
		 * --------------------------
		 */
		else if (!Arrays.equals(this.getJPasswordField().getPassword(),
			this.getJPasswordConfirmPassword().getPassword())) {
		 ...
		 */
		    ...
		    String msg = Language.translate("The password should be at least 6 characters in length!",
			    Language.EN);
		...
		/*
		 * ------------------------------------------------------- ----- Verify if the
		 * Country code is valid ------------- ------------------ ISO code
		 * ---------------------------
		 */
...
//					String alias = this.getJTextFieldAlias().getText();
			...
			File keyStoreFile = new File(
				jFileChooserOpen.getSelectedFile().getAbsoluteFile().getAbsolutePath() + File.separator
					+ keystoreName + HttpsConfigWindow.KEYSTORE_FILENAME);

			// ---- Create the KeyStore with informations entered by the User ---------
			getKeyStoreController().createKeyStore(certificateProperties, keyStoreFile, keystorePassword,
				keyStoreValidity);
			...
			getKeyStoreController().openTrustStore(this.httpsConfigWindow.getKeyStoreFile(),
				keystorePassword);
			CertificateProperties keyStoreSettings = getKeyStoreController()
				.getFirstCertificateProperties();

			fillFields(keyStoreSettings);

			this.httpsConfigWindow.getJTextFieldKeyStoreLocationPath()
				.setText(keyStoreFile.getAbsolutePath());
		...
		// ---- Update KeyStore ---------------------------
		// ---- Verify if all the fields are filled -------
		if (this.getJTextFieldAlias().getText().equals("") || this.getJPasswordField().getPassword().length == 0
			|| this.getJPasswordConfirmPassword().getPassword().length == 0) {
		    String msg = Language.translate("You must fill out all required fields!", Language.EN);
		    String title = Language.translate("Required fields", Language.EN);
		    JOptionPane.showMessageDialog(this, msg, title, JOptionPane.WARNING_MESSAGE);
		 ...
		 */
			...
			if (this.httpsConfigWindow.getKeyStorePassword().equals(oldPassword)) {
			    ...
			    getKeyStoreController().editLonelyKeyEntry(this.httpsConfigWindow.getKeyAlias(),
				    newKeyAlias, newKeyStorePassword, this.httpsConfigWindow.getKeyStorePassword());
			    String msg1 = Language.translate("Your keystore has been updated successfully!",
				    Language.EN);
			    ...
			    this.getJPasswordConfirmPassword().setText(null);
			    this.httpsConfigWindow
				    .setKeyAlias(getKeyStoreController().getFirstCertificateProperties().getAlias());
			} else {
			    String msg2 = Language.translate("The password you entered is incorrect. Please try again!",
				    Language.EN);
		...
		int option = JOptionPane.showOptionDialog(null, getJPanelCertificate(), title, JOptionPane.NO_OPTION,
			JOptionPane.PLAIN_MESSAGE, null, options, jTextFieldCertificateName);
		if (option == 0) {
		    if (this.httpsConfigWindow.getKeyStoreFile() == null) {
			String msg = Language.translate("Please open a Keystore first!", Language.EN);
...
//						getJTextFieldCertificatePath().setText(null);
			...
			String certificatePath = getJTextFieldCertificatePath().getText() + File.separator
				+ getJTextFieldCertificateName().getText();
