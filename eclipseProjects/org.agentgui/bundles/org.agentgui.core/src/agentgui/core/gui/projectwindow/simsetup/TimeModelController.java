...
/**
 * The Class TimeModelController is used within Project and manages the display
 * of TimeModel's
 * 
 * @see Project
 * @see TimeModel
...
public class TimeModelController implements Observer {
	...
	boolean isDifferentClass = timeModel != null && this.currProject.getTimeModelClass() != null
		&& this.currProject.getTimeModelClass().equals(timeModel.getClass().getName()) == false;
	...
	boolean isDifferentSetup = currentSetup == null
		|| currentSetup.equals(currProject.getSimulationSetupCurrent()) == false;
	...
	if (configPanel != null) {
	    this.pwt = new ProjectWindowTab(this.currProject, ProjectWindowTab.DISPLAY_4_END_USER,
		    Language.translate("Zeit-Konfiguration"), null, null, configPanel,
		    Language.translate(ProjectWindowTab.TAB_4_SUB_PANES_Setup));
	...
	if (newIndexPosistion != this.indexPositionOfTimeModelTab && this.pwt != null
		&& this.getJPanel4TimeModelConfiguration() != null) {
		...
		try {
		    ...
		    System.err.println("[" + this.getClass().getSimpleName()
			    + "] The Tab for the TimeModel configuration could not pe placed on new index positon "
			    + newIndexPosistion + "!");
