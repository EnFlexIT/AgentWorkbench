...
/**
 * This JPanel allows the user to : 1- create new TrustStore and protect its
 * integrity with a password 2- update his TrustStore informations 3- add or
...
public class TrustStoreConfigPanel extends JPanel implements ActionListener, MouseListener {
	...
	if (ae.getSource() == this.getJButtonApplyTrustStore()) {
		...
		if (this.getJTextFieldTrustStoreName().getText().equals("")
			|| this.getJPasswordFieldPassword().getPassword().length == 0
			|| this.getJPasswordFieldConfirmPassword().getPassword().length == 0) {
		 ...
		 * --------------------------
		 */
		else if (!Arrays.equals(this.getJPasswordFieldPassword().getPassword(),
			this.getJPasswordFieldConfirmPassword().getPassword())) {
		 ...
		 */
		    ...
		    String trustStoreName = this.getJTextFieldTrustStoreName().getText()
			    + HttpsConfigWindow.TRUSTSTORE_FILENAME;
			...
			this.httpsConfigWindow.setTrustStoreFile(
				new File(jFileChooser.getSelectedFile().getAbsoluteFile().getAbsolutePath()
					+ File.separator + trustStoreName));
			this.getTrustStoreController().createTrustStore(this.httpsConfigWindow.getTrustStoreFile(),
			...
			this.httpsConfigWindow.getJTextFieldTrustStoreLocationPath()
				.setText(this.httpsConfigWindow.getTrustStoreFile().getAbsolutePath());
			...
			this.getJTextFieldTrustStoreName()
				.setText(this.httpsConfigWindow.getTrustStoreFile().getAbsolutePath());
		 ...
		 */
			...
			if (this.httpsConfigWindow.getTrustStorePassword().equals(oldPassword)) {
			    ...
			    String msg = Language.translate("Your TrustStore has been updated successfully!",
				    Language.EN);
			    ...
			    String msg = Language.translate("The password you entered is incorrect. Please try again!",
				    Language.EN);
		...
		if (result == JFileChooser.APPROVE_OPTION) {
		    ...
		    JLabel jLabelCertificate = new JLabel(
			    Language.translate("Please, enter Certificate's alias ", Language.EN) + ":");
			...
			if (certificateAlias.equals("")) {
			    ...
			    // ------------ Press OK Button --------------------
			    this.getTrustStoreController().addCertificateToTrustStore(certificateToAdd,
				    certificateAlias);
			    this.getTrustStoreController().clearTableModel();
			    this.getTrustStoreController().getTrustedCertificatesList();
			    this.getjTableTrusTedCertificates()
				    .setModel(this.getTrustStoreController().getTableModel());
			}
		    }
		} else {
		...
		if (getjTableTrusTedCertificates().getSelectedRow() == -1) {
		    ...
		    String title = Language.translate("Warning message", Language.EN);
		    int option = JOptionPane.showOptionDialog(null, msg, title, JOptionPane.NO_OPTION,
	...
	if (e.getSource() == this.getjTableTrusTedCertificates()) {
		...
		CertificateProperties certificateProperties = this.getTrustStoreController()
			.getCertificateProperties(alias);
