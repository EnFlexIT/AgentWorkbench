...
public class JPanelEmbeddedSystemAgentTable extends JPanel implements ActionListener {
     ...
     * 
     * @param deviceServiceAgents configuration value for agents that are to be
     *                            executed, includes agentname and agent cass names
     */
	...
	if (project == null) {
	    ...
	    this.esaClassSelector = new ClassSelectionDialog(this.optionDialog, jListClassSearcher, currAgentClass,
		    null, Language.translate("Bitte wählen Sie den Agenten aus, der gestartet werden soll"), false);
	    this.esaProject = project;
	}
	...
	if (ae.getSource() == this.getJButtonAddAgent()) {
	    ...
	    if (selectedModelRowIndex == -1) {
		String msgHead = Language.translate("Fehlende Auswahl!");
		String msgText = Language
			.translate("Bitte wählen Sie die Zeile, in der ein Agent definiert werden soll") + "!";
		JOptionPane.showMessageDialog(this.optionDialog.getContentPane(), msgText, msgHead,
			JOptionPane.WARNING_MESSAGE);
