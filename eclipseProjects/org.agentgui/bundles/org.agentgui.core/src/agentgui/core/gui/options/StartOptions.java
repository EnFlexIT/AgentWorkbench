...
public class StartOptions extends AbstractOptionTab implements ActionListener {
	...
	// --- Translate ----------------------------------
	jLabelRunsAs
		.setText(Application.getGlobalInfo().getApplicationTitle() + " - " + Language.translate("Starte als:"));
	...
	if (newCompPrefHeight > newCompHeight)
	    newCompHeight = newCompPrefHeight;
	...
	if (this.executionModeNew != this.executionModeOld) {
	    ...
	    // ----------------------------------------------------------------
	    String executionModeTextNew = Application.getGlobalInfo()
		    .getExecutionModeDescription(this.executionModeNew);
	    String msgHead = "";
	    String msgText = "";

	    msgHead += Application.getGlobalInfo().getApplicationTitle() + ": "
		    + Language.translate("Anwendung umschalten ?");
	    ...
	    int msgAnswer = JOptionPane.showConfirmDialog(this.optionDialog, msgText, msgHead,
	...
	if (this.executionModeNew == this.executionModeOld) {
		...
		System.out.println("\n" + Language.translate("Neustart") + " "
			+ Application.getGlobalInfo().getExecutionModeDescription(this.executionModeNew) + " ...");
		Application.getJadePlatform().stop(false);
		if (Application.getProjectsLoaded() != null) {
		    if (Application.getProjectsLoaded().closeAll(this.optionDialog, true) == false)
			return;
		}
	    ...
	    String executionModeTextOld = Application.getGlobalInfo()
		    .getExecutionModeDescription(this.executionModeOld);
	    String textMiddle = Language.translate("auf");
	    String executionModeTextNew = Application.getGlobalInfo()
		    .getExecutionModeDescription(this.executionModeNew);
	    System.out.println(
		    textPrefix + " '" + executionModeTextOld + "' " + textMiddle + " '" + executionModeTextNew + "'");
		...
		// --- Application Modus ----------------------------
		if (Application.getProjectsLoaded() != null) {
		    if (Application.getProjectsLoaded().closeAll(this.optionDialog) == false)
			return;
		}
	...
	if (actCMD.equalsIgnoreCase("runAsApplication") || actCMD.equalsIgnoreCase("runAsServer")
		|| actCMD.equalsIgnoreCase("runAsEmbeddedSystemAgent")) {
	    ...
	    if (getJPanelMTPConfig().getJComboBoxMtpProtocol().getSelectedProtocol() != getJPanelMasterConfiguration()
		    .getJcomboboxMtpProtocol().getSelectedProtocol()) {
		String title = Language.translate("Different MTP-protocols configured!", Language.EN);
		String msg = Language.translate(
			"Please, choose the same Protocol for the server.master and the MTP-protocol!", Language.EN);
