...
public class StartSetup extends JPanel implements Observer, ActionListener {
	...
	if (jListStartList == null) {
		...
		@Override
		public Component getListCellRendererComponent(JList<? extends AgentClassElement4SimStart> list,
			AgentClassElement4SimStart value, int index, boolean isSelected, boolean cellHasFocus) {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			if (agentSelected.equals(agentSelectedLast) == false) {
			    ...
			    OntologyInstanceViewer oiv = new OntologyInstanceViewer(
				    currProject.getOntologyVisualisationHelper(),
				    currProject.getAgentStartConfiguration(), agentSelected.getAgentClassReference());
	...
	this.jListModelAgents2Start = this.currSimSetup.getAgentDefaultListModel(this.jListModelAgents2Start,
		SimulationSetup.AGENT_LIST_ManualConfiguration);
	...
	if (agentSelector.isCanceled() == false) {
		...
		AgentClassElement4SimStart ac4s = new AgentClassElement4SimStart(
			(ClassElement2Display) agentsSelected[i], SimulationSetup.AGENT_LIST_ManualConfiguration);
    ...
    /**
     * This Method changes the postion / order of a selected single agent in the
	...
	if (jListStartList.getSelectedValuesList().size() == 1) {
	    ...
	    ac4s = jListStartList.getSelectedValue();
	...
	for (int i = 0; i < jListModelAgents2Start.size(); i++) {
	    ac4s = jListModelAgents2Start.getElementAt(i);
	    ac4s.setPostionNo(startCounter);
	    startCounter++;
	}
	...
	if (updateObject instanceof SimulationSetupNotification) {
		...
		this.currSimSetup.setAgentDefaultListModel(SimulationSetup.AGENT_LIST_ManualConfiguration,
			this.jListModelAgents2Start);
		this.setupLoad();
		break;
	    default:
		this.setupLoad();
	...
	if (trigger == jComboBoxStartLists) {
		...
		DefaultListModel<AgentClassElement4SimStart> dlm = this.currSimSetup
			.getAgentDefaultListModel(selectedItem);
