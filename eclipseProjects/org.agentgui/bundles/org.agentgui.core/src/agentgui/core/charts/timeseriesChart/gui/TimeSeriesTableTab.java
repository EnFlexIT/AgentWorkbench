...
public class TimeSeriesTableTab extends TableTab {
     ...
     * 
     * @param model             {@link TimeSeriesDataModel}
     * @param parentChartEditor {@link ChartEditorJPanel}
     */
	...
	if (jTable == null) {
		...
		sorter.setComparator(0, new Comparator<Number>() {
			...
			if (value1 instanceof Float || value1 instanceof Double || value2 instanceof Float
...
//			}
	...
	if (e.getSource() == getBtnAddColumn() || (e.getSource() == getBtnAddRow() && getTable().getRowCount() == 0)) {
	    ...
	    String seriesLabel = (String) JOptionPane.showInputDialog(this, Language.translate("Label"),
		    Language.translate("Neue Datenreihe"), JOptionPane.QUESTION_MESSAGE, null, null,
		    dataModelLocal.getDefaultSeriesLabel());
	    if (seriesLabel != null) {
		// --- As JFreeChart doesn't work with empty data series, one value pair must be
		// added ------
		...
		if (this.dataModelLocal.getSeriesCount() > 0) {
			...
			initialValuePair = this.dataModelLocal.createNewValuePair(
				(Number) this.dataModelLocal.getTableModel().getValueAt(row, keyColumn), 0);
