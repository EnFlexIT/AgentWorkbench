...
public class XyTableModel extends TableModel {
	...
	if (tableModelDataVector.size() == 0) {
	    ...
	    this.getOntologyModel().addXyValuePair(this.getFocusedSeriesIndex(), modelLineSelectedNew, newXValue,
		    newYValue);
	    this.getChartModel().addXyDataItem(this.getFocusedSeriesIndex(), modelLineSelectedNew, newXValue,
		    newYValue);
     ...
     * @throws NoSuchSeriesException Will be thrown if there is no series with the
     *                               specified index
     */
    @Override
	...
	if (tme.getSource() == this && tme.getFirstRow() >= 0) {
		...
		try {
		    ...
		    // --- Do we have changes? ------------------------------------------
		    if (colIndex == 1) {
			if (((Float) this.latestChangedValue).equals(xValue))
			    return;
			if (this.getOntologyModel().getSeries(this.getFocusedSeriesIndex())
				.getAvoidDuplicateXValues() == true) {
				...
				JOptionPane.showMessageDialog(this.myJTable, Language.translate(msg),
					Language.translate(title), JOptionPane.WARNING_MESSAGE, null);
			...
			if (((Float) this.latestChangedValue).equals(yValue))
			    return;
		    }
			...
			this.getOntologyModel().updateXyValuePair(this.getFocusedSeriesIndex(), rowIndex, xValue,
				yValue);
		    ...
		    System.err.println(
			    "Error updating data model: Series " + this.getFocusedSeriesIndex() + " does mot exist!");
		    e.printStackTrace();
		}
	...
	if (this.tableModelDataVector != null && this.tableModelDataVector.size() != 0) {
	    ...
	    XyDataSeries xyDataSeriesCurrent = (XyDataSeries) this.getOntologyModel().getChartData()
		    .get(this.getFocusedSeriesIndex());
	...
	if (jTable.getSelectedRow() != -1 && direction != 0) {
	    ...
	    if (!((selectedRowModel == 0 && direction < 0)
		    || (selectedRowModel == this.tableModelDataVector.size() - 1 && direction > 0))) {
		Vector<Number> rowSelected = this.tableModelDataVector.remove(selectedRowModel);
		XyValuePair xyValuePair = this.getOntologyModel().removeXyValuePair(this.getFocusedSeriesIndex(),
			selectedRowModel);
		...
		if (direction > 0) {
		    ...
		    this.getOntologyModel().addXyValuePair(this.getFocusedSeriesIndex(), selectedRowModelNew,
			    xyValuePair);
		    this.getChartModel().setXYSeriesAccordingToOntologyModel(this.getFocusedSeriesIndex());
