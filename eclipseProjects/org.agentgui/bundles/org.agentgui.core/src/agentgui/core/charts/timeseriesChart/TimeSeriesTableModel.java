...
public class TimeSeriesTableModel extends TableModel {
	...
	this.tableModelDataVector = new TableModelDataVector(TimeSeries.class.getSimpleName(),
		this.tableModelDataVector.isActivateRowNumber(), this.tableModelDataVector.getKeyColumnIndex());
	...
	for (int i = 0; i < valuePairs.size(); i++) {
		...
		if (rowIndex >= 0) {
			...
			double compareValue = tableModelDataVector.get(j).get(0).doubleValue();
	...
	// Append empty values to all rows for which the new series did not contain a
	// value
...
//			}
	...
	for (int i = 0; i < getRowCount(); i++) {
	    ...
	    Number keyValue = rowVector.get(0);
	...
	if (targetDataSeriesIndex <= (this.getColumnCount() - 1)) {
	    ...
	    int targetTbIndex = targetDataSeriesIndex + 1;
	    TreeMap<Number, Vector<Number>> tableDataTreeMap = new TreeMap<Number, Vector<Number>>(
		    this.tableModelDataVector.getKeyRowVectorTreeMap());
     ...
     * @param series                the series
     * @param targetDataSeriesIndex the target data series index
     * @throws agentgui.core.charts.NoSuchSeriesException thrown when trying to
     *                                                    access a data series that
     *                                                    does not exist
     */
	...
	this.addOrUpdateValuePair(targetDataSeriesIndex, valuePair.getTimestamp().getLongValue(),
		valuePair.getValue().getFloatValue());
    }
		...
		if (tme.getColumn() > 0) {
			...
			if (value != null) {
				...
				for (int i = 1; i < rowVector.size(); i++) {
				    Number seriesValue = rowVector.get(i);
				    int series = i - 1;
				    if (seriesValue != null) {
