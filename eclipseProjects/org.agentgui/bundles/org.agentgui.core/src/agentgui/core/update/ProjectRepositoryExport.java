...
/**
 * The Class ProjectRepositoryExport manages the export to the globally
 * specified project repository directory.
 * 
...
public class ProjectRepositoryExport extends Thread {
	...
	if (repositoryLocationDirectoryPath == null || repositoryLocationDirectoryPath.isEmpty() == true) {
	    ...
	    repositoryLocationDirectoryPath = Application.getGlobalInfo()
	...
	if (configError != null) {
		...
		JOptionPane.showMessageDialog(Application.getMainWindow(), errMsg, "Repository Export Error",
	...
	if (destinDir.exists() == false) {
	    destinDir.mkdirs();
	    if (destinDir.exists() == false) {
		System.err.println("[" + this.getClass().getSimpleName() + "] Could not create directory '"
			+ this.getRepositoryLocationDirectoryPath() + "'.");
		return;
	    }
	}

	// --- Export the project -----------------------------------
	this.getProjectExportController().exportProject(this.currProject, this.getProjectExportSettings(),
		this.isShowUserDialogs(), false);
	...
	String fileName = project.getProjectFolder() + "_" + project.getVersionTag() + "_"
		+ project.getVersion().toString() + "." + Application.getGlobalInfo().getFileEndProjectZip();
