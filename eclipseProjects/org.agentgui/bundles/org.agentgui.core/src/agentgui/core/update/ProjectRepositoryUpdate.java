...
public class ProjectRepositoryUpdate extends Thread {
	...
	if (this.isUserRequestForDownloadAndInstallation() == true) {
		...
		int answer = JOptionPane.showConfirmDialog(Application.getMainWindow(), message, title,
	...
	if (this.isLeaveUpdateProcedure() == null)
	    this.setLeaveUpdateProcedure(localLeaveUpdateProcedure);
	if (this.isUserRequestForDownloadAndInstallation() == null)
	    this.setUserRequestForDownloadAndInstallation(localUserRequestForDownloadAndInstallation);
	if (this.isUserRequestForInstallation() == null)
	    this.setUserRequestForInstallation(localUserRequestForInstallation);
	if (this.isShowFinalUserMessage() == null)
	    this.setShowFinalUserMessage(localShowFinalUserMessage);

	// --- If AWB is executed from IDE, skip the update ---------
	if (this.debugUpdateProcedure == false
		&& Application.getGlobalInfo().getExecutionEnvironment() == ExecutionEnvironment.ExecutedOverIDE) {

	    String message = Language.translate("No project updates are available in the IDE environment.",
		    Language.EN);
	    if (this.isHeadlessUpdate() == false && this.isExecutedByUser() == true) {
		JOptionPane.showMessageDialog(Application.getMainWindow(), message, "IDE - Environment",
    ...
    /**
     * Does the same as calling the threads {@link #start()} method, but without
     * starting an individual thread. Thus, it enables to wait until the project
     * update is finalized.
     */
	...
	if (this.isLeaveUpdateProcedure() == true)
	    return;
	...
	if (this.currProject.getUpdateSite() == null || this.currProject.getUpdateSite().isEmpty() == true) {
	    this.printSystemOutput(
		    "No update-site was specified for the project '" + this.currProject.getProjectName() + "'!", true);
	    return;
	}
	...
	try {
		...
		if (this.isConfirmedUserRequestForDownloadAndInstallation(update) == false)
		    return;

		// --- Download the update ------------------------------
		String updateFilename = this.getLinkOrPathWithDirectorySuffix(
			Application.getGlobalInfo().getPathProjects(true), update.getFileName());
		if (this.downloadOrCopyProjectArchiveFromRepository(this.currProject.getUpdateSite(), update,
			updateFilename, this.currProject.getUpdateAuthorization()) == true) {
		    // --- The download of update is done ---------------
		    if (this.isConfirmedUserRequestForInstallation(update) == false)
			return;
			...
			JOptionPane.showMessageDialog(Application.getMainWindow(), updateMessage, updateTitle,
				updateMessageType);
		    } else {
		...
		// --- No Update found ----------------------------------
		updateTitle = Language.translate("Updated check for", Language.EN) + " '"
			+ this.currProject.getProjectName() + "'";
	    ...
	    if (this.isHeadlessUpdate() == false && this.isShowFinalUserMessage() == true) {
		JOptionPane.showMessageDialog(Application.getMainWindow(), ex.getMessage(), updateTitle,
			updateMessageType);
	    } else {
	...
	if (this.currProject.close(null, true) == false)
	    return false;

	// --- Backup the old project into an archive ---------------
	if (this.packCurrentProjectToArchive() == false)
	    return false;
    ...
    /**
     * Returns the task that is to be executed after the import of the project
     * update.
     * 
    ...
    /**
     * Restores the previously configured update settings after updating the
     * project.
     * 
    ...
    /**
     * Packs the current project into a project archive located in the projects
     * directory and removes the project.
     * 
     * @return true, if successful
     */
     ...
     *
     * @param sourceDirectoryOrWebReference the source directory or web reference
     *                                      (without any file)
     * @param updateRepositoryEntry         the update repository entry
     * @param destinationFileName           the destination file name
     * @param webResAuth                    the web resource authorization
     * @return true, if successful
     * @throws ProjectRepositoryUpdateException if project repository update fails
     */
    public boolean downloadOrCopyProjectArchiveFromRepository(String sourceDirectoryOrWebReference,
	    RepositoryEntry updateRepositoryEntry, String destinationFileName, WebResourcesAuthorization webResAuth)
	    throws ProjectRepositoryUpdateException {
    ...
    /**
     * Return the download file name URL base on the specified
     * {@link RepositoryEntry}.
     *
     * @param sourceDirectoryOrWebReference the source directory or web reference
     *                                      (without any file)
     * @param updateRepositoryEntry         the update repository entry
	...
	return this.getLinkOrPathWithDirectorySuffix(sourceDirectoryOrWebReference, "/")
		+ updateRepositoryEntry.getFileName();
    }
	...
	return this.getLinkOrPathWithDirectorySuffix(sourceDirectoryOrWebReference, File.separator)
		+ updateRepositoryEntry.getFileName();
    }
	...
	if (projectRepository == null && this.currProject.getUpdateSite() != null) {
	    projectRepository = ProjectRepository.loadProjectRepository(this.currProject.getUpdateSite(),
		    this.currProject.getUpdateAuthorization());
