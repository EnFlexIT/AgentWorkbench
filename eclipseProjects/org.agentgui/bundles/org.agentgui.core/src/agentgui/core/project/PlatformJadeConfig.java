...
public class PlatformJadeConfig implements Serializable {
    ...
    /**
     * The enumeration MTP_Creation describes the possibilities, how the MTP-address
     * can be configured.
     */
    public static enum MTP_Creation {
	ConfiguredByJADE, ConfiguredByIPandPort
    }
    ...
    public static final String SERVICE_AgentManagementService = jade.core.management.AgentManagementService.class
	    .getName();
    ...
    public static final String SERVICE_MainReplicationService = jade.core.replication.MainReplicationService.class
	    .getName();
    public static final String SERVICE_FaultRecoveryService = jade.core.faultRecovery.FaultRecoveryService.class
	    .getName();
    public static final String SERVICE_AddressNotificationService = jade.core.replication.AddressNotificationService.class
	    .getName();
    public static final String SERVICE_TopicManagementService = jade.core.messaging.TopicManagementService.class
	    .getName();
    public static final String SERVICE_PersistentDeliveryService = jade.core.messaging.PersistentDeliveryService.class
	    .getName();
    public static final String SERVICE_UDPNodeMonitoringServ = jade.core.nodeMonitoring.UDPNodeMonitoringService.class
	    .getName();
    ...
    public static final String SERVICE_AgentGUI_SimulationService = agentgui.simulationService.SimulationService.class
	    .getName();

    // --- Add-On-Services ----------------------------------------------------
    public static final String SERVICE_InterPlatformMobilityService = jade.core.migration.InterPlatformMobilityService.class
	    .getName();
    ...
    /**
     * This Method returns a new Instance of {@link ProfileImpl} that is used to
    ...
    /**
     * Adds the configured 'LOCAL_HOST' to the input instance of Profile in case
     * that the localhost hosts the server.master too. In that case the platform URL
     * should exactly match the configuration.
     * 
     * @param profile the profile to work on
     */
	...
	if (mtpCreation == MTP_Creation.ConfiguredByIPandPort) {
		...
		if (this.isAvailableNetworkAddress(mtpIpAddress) == false) {
		    ...
		    ipAddress = this.getAutomaticIP();
		    System.err.println("=> The configured IP address '" + mtpIpAddress
			    + "' is invalid! The JADE profile will be corrected with the available IP '" + ipAddress
			    + "' instead.");
		} else {
		...
		if (mtpProtocol == MtpProtocol.HTTP) {
		    ...
		    profile.setParameter(Profile.MTPS, jade.mtp.http.MessageTransportProtocol.class.getName()
			    + "(http://" + ipAddress + ":" + freePort + "/acc)");
		    ...
		    profile.setParameter("jade_mtp_http_https_trustManagerClass",
			    jade.mtp.http.https.FriendListAuthentication.class.getName());
		    ...
		    // reset LOCAL_HOST and set it to to loopback to not open the RMI ports (1099)
	...
	if (this.getProject() == null && globalInfo.getJadeUrlConfigurationForMaster().isLocalhost()) {
		...
		// --------------------------------------------------------
		Integer freePort = new PortChecker(globalInfo.getServerMasterPort4MTP(),
			globalInfo.getServerMasterURL()).getFreePort();
	...
	if (this.getServiceList().contains(serviceClassReference) == false
		&& serviceClassReference.contains(getAutoServiceTextAddition()) == false) {
    ...
    /**
     * With this class the LocalPort, which will be used from a JADE-Container can
     * be set.
     * 
    ...
    /**
     * Returns the current Port which is configured for the MTP of the JADE main
     * container.
     * 
    ...
    /**
     * This Method returns a String which shows the current configuration of this
     * instance.
     *
     * @return String
     */
