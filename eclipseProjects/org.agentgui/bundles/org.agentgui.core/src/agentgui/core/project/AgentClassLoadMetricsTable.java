...
/**
 * The Class AgentClassLoadMetricsTable describes the predictive or real
 * measured metrics of the. agent classes within a project.
 * 
...
public class AgentClassLoadMetricsTable implements Serializable {
     ...
     * 
     * @param agentClassMetricDescriptionVector the new agent load description
     *                                          vector
     */
     ...
     * @param realMetricAverage  the real metric average
     */
    public void addAgentLoadDescription(String className, double userPredictiveLoad, double realMetric,
	    double realMetricMin, double realMetricMax, double realMetricAverage) {
	this.getAgentClassMetricDescriptionVector().addElement(new AgentClassMetricDescription(currProject, className,
		userPredictiveLoad, realMetric, realMetricMin, realMetricMax, realMetricAverage));
	this.setProjectNotification(Project.AGENT_METRIC_AgentDescriptionAdded);
    }
	...
	for (int i = 0; i < this.agentClassMetricDescriptionVector.size(); i++) {
	    this.agentClassMetricDescriptionVector.get(i)
		    .setRealMetric(this.agentClassMetricDescriptionVector.get(i).getRealMetricAverage());
	}
    }
