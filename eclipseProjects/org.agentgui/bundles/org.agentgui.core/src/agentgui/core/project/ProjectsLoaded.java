...
/**
 * This class holds the list of the projects, that are currently open within
 * Agent.GUI and offers methods to deal with them.
 * 
...
public class ProjectsLoaded {
     ...
     * 
     * @param addNew the add new. If set false, the user dialog for selecting a
     *               project to open will be shown
     ...
     *
     * @param selectedProjectFolder the selected project folder from the projects
     *                              directory
     ...
     * @return The Project-instance that was added here
     */
    public Project add(boolean addNew, String selectedProjectFolder, MApplication mApplication,
	    EPartService ePartService, EModelService eModelService) {
	...
	if (selectedProjectFolder == null) {
	    // --- Open user dialog -------------------------------------------
	    AwbProjectNewOpenDialog newProDia = UiBridge.getInstance().getProjectNewOpenDialog(
		    Application.getGlobalInfo().getApplicationTitle() + ": " + actionTitel, action);
	    newProDia.setProjectName(projectNameTest);
	    newProDia.setProjectDirectory(projectFolderTest);
	    newProDia.setVisible(true);
	    // === Wait for user here =========================================
	    if (newProDia.isCanceled() == true || newProDia.getProjectDirectory() == null
		    || newProDia.getProjectDirectory().isEmpty() == true) {
	...
	Application.informApplicationListener(
		new ApplicationEvent(ApplicationEvent.PROJECT_LOADING_PROJECT_FILES_LOADED, newProject));
	...
	newProject.getSimulationSetups().setupLoadAndFocus(SimNoteReason.SIMULATION_SETUP_LOAD,
		newProject.getSimulationSetupCurrent(), false);
     ...
     * This method will try to close all open projects.
     * 
     * @param parentComponent the parent component (for messages by using the
     *                        {@link JOptionPane})
     * @return Returns true on success
     */
     ...
     * @param parentComponent the parent component (for messages by using the
     *                        {@link JOptionPane})
     * @param isSkipSaving    set true, if you want to skip user interactions for
     *                        saving a project
     * @return Returns true on success
     */
     ...
     * 
     * @param indexOfProject index of the project
     * @return A Project instance
     */
     ...
     * 
     * @param project2Remove project, which should be removed
     */
    ...
    /**
     * Identifies a Project by its Root-Folder-Name and returns the
     * Array-/Window-Index
     * 
     * @param projectFolderName name of the project folder
     * @return The index position of a project
     */
    public int getIndexByFolderName(String projectFolderName) {
	int index = -1;
	for (int i = 0; i < this.count(); i++) {
	    if (this.getProjectsOpen().get(i).getProjectFolder().toLowerCase()
		    .equalsIgnoreCase(projectFolderName.toLowerCase())) {
		index = i;
		break;
	    }
	}
     ...
     * 
     * @return count of opened projects
     */
    public int count() {
    ...
    /**
     * Configures the appearance of the application, depending on the current
	...
	FileNameExtensionFilter filter = new FileNameExtensionFilter(
		Language.translate("Agent.GUI Projekt-Datei") + " (*." + fileEnd + ")", fileEnd);
	...
	int answerChooser = fileChooser.showDialog(Application.getMainWindow(),
		Language.translate("Projekt importieren"));
	...
	AwbProjectNewOpenDialog newProDia = UiBridge.getInstance().getProjectNewOpenDialog(
		Application.getGlobalInfo().getApplicationTitle() + ": " + actionTitle, ProjectAction.ExportProject);
	newProDia.setVisible(true);
	...
	String repositoryPath = Application.getGlobalInfo()
		.getStringFromConfiguration(BundleProperties.DEF_LOCAL_PROJECT_REPOSITORY, null);
	...
	if (projectExportDialog.isCanceled() == true)
	    return;
	...
	AwbProjectNewOpenDialog newProDia = UiBridge.getInstance().getProjectNewOpenDialog(
		Application.getGlobalInfo().getApplicationTitle() + ": " + actionTitel, ProjectAction.DeleteProject);
	newProDia.setVisible(true);
	...
	if (Application.getProjectFocused() != null) {
		...
		optionMsg = currProject.getProjectName() + ": "
			+ Language.translate("Das Projekt wird nun geschlossen!");
	...
	if (exportBeforeDelete == true) {
	    ...
	    DefaultProjectExportController exportController = (DefaultProjectExportController) ProjectExportControllerProvider
		    .getProjectExportController();
    ...
    /**
     * Deletes the specified project sub directory (the last element in the path is
     * enough).
     * 
	...
	public ProjectDeleteThread(Project project, DefaultProjectExportController exportController,
