...
public class SimulationSetups extends TreeMap<String, String> {
	...
	if (this.containsKey(setupName) == false)
	    return;
	...
	if (this.containsKey(nameOld) == false)
	    return;
	...
	List<File> setupFileListNew = SimulationSetup
		.getSetupFiles(new File(this.currProject.getSubFolder4Setups(true) + fileNameNew));
     ...
     * @param isAlreadyLoadedSetupInstance indicator, that tell the method if the
     *                                     setup instance is already loaded or not.
     *                                     If <code>false</code>, the setup files
     *                                     will be loaded.
    ...
    /**
     * This method loads the current Simulation-Setup to the local variable
     * 'currSimSetup' which can be get and set by using {@link #getCurrSimSetup()}
     * or {@link #setCurrSimSetup(SimulationSetup)}.
     */
	...
	if (setupXmlFile.exists() == true) {
		...
		Application.informApplicationListener(
			new ApplicationEvent(ApplicationEvent.PROJECT_LOADING_SETUP_FILES_LOADED, this.currSimSetup));
	    }
	    ...
	    String msg = Language.translate("Die Datei") + " '" + this.get(currSimSetupName) + "' "
		    + Language.translate("für das Setup") + " '" + currSimSetupName + "' "
		    + Language.translate("wurde nicht gefunden.");
	    ...
	    int answer = JOptionPane.showConfirmDialog(Application.getMainWindow(), msg, head,
    ...
    /**
     * This Method checks if the incoming Setup-Name is already used in the current
     * List of Setups (HashMap).
     *
	...
	if (this.currSimSetup != null) {
	    ...
	    this.currProject.setChangedAndNotify(
		    new SimulationSetupNotification(SimNoteReason.SIMULATION_SETUP_PREPARE_SAVING));
	    ...
	    this.currProject
		    .setNotChangedButNotify(new SimulationSetupNotification(SimNoteReason.SIMULATION_SETUP_SAVED));
	}
	this.setupCleanUpSubFolder();
    }

    /**
     * This Method scans the Folder of the Simulation-Setups and deletes all file,
    ...
    /**
     * Sets the current {@link SimulationSetup}'s xml file name as String (full
     * path).
     * 
     * @param xmlFileNameFullPath the new setup file name for the XML file (full
     *                            path)
     */
    ...
    /**
     * Returns the current {@link SimulationSetup}'s xml file name as String (full
     * path).
     * 
	...
	if (setupUserFileXml.exists() == true)
	    fileList.add(setupUserFileXml);
	if (setupUserFileBin.exists() == true)
	    fileList.add(setupUserFileBin);
