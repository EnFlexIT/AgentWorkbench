...
public class SimulationSetup {
    ...
    /**
     * This method saves the current {@link SimulationSetup} and it files to the
     * default location for setups.<br>
	...
	if (setupXmlFile == null) {
	    System.err.println("[" + this.getClass().getSimpleName()
		    + "] The path for saving the setups user runtime object is not allowed to be null!");
	    return false;
	}
	...
	if (this.getUserRuntimeObject() == null) {
		...
		try {
		    fos = new FileOutputStream(
			    SimulationSetup.getSetupFile(setupXmlFile, SetupFileType.USER_OBJECT_BIN));
		...
		System.out.println("[" + this.getClass().getSimpleName()
			+ "] Error while saving the setups user runtime object as bin file:");
     ...
     * 
     * @param setupXmlFile          the setup xml file
     * @param loadUserRuntimeObject indicates if the user runtime object should be
     *                              loaded
	...
	try {
	    ...
	    setup.getApplicationEventsToFire()
		    .add(new ApplicationEvent(ApplicationEvent.PROJECT_LOADING_SETUP_XML_FILE_LOADED, setup));
    ...
    /**
     * Loads the {@link SimulationSetup}'s user runtime object from the specified
     * file.
     *
	...
	if (setup != null && setup.getUserRuntimeObjectClassName() != null) {
		...
		if (userObject != null) {
		    ...
		    // --- Fire application event ---------
		    setup.getApplicationEventsToFire()
			    .add(new ApplicationEvent(ApplicationEvent.PROJECT_LOADING_SETUP_USER_FILE_LOADED, setup));
		}
    ...
    /**
     * This method will create all DefaultListModels which will be used within the
     * visible application<br>
     * as for example for the manual agent configuration (tab 'Simulation-Setup'
     * =&gt; 'Agent-Start')<br>
     * or the configuration of the environment model (tab 'Simulation-Setup' =&gt;
     * 'Simulation Environment').<br>
     * The resulting ListModels can be get by using
     * {@link #getAgentDefaultListModel(String)}
     */
     ...
     */
    public void setAgentDefaultListModel(String listName,
	    DefaultListModel<AgentClassElement4SimStart> defaultListModel4AgentStarts) {
	if (listName != null) {
	...
	for (int i = 0; i < dlm.getSize(); i++) {
	    agentLists.add(dlm.getElementAt(i));
	}
	// --- If the default list is not there, create it ----------
	if (agentLists.contains(AGENT_LIST_ManualConfiguration) == false) {
	    this.getAgentDefaultListModel(new DefaultListModel<AgentClassElement4SimStart>(),
    ...
    /**
     * This method can be used in order to add an individual agent start list to the
     * SimulationSetup.<br>
     * The list will be filled with elements of the type
     * {@link AgentClassElement4SimStart} coming from the stored setup file and will
     * be later on also stored in the file of the simulation setup.
     *
     * @see AgentClassElement4SimStart AgentClassElement4SimStart - The type to use
     *      within a concrete list model
     * 
     * @param newDefaultListModel4AgentStarts the new DefaultListModel to set
     * @param listName                        the name of the list to be assigned.
     *                                        Consider the use of one of the
     *                                        constants
     *                                        {@link #AGENT_LIST_ManualConfiguration}
     *                                        or
     *                                        {@link #AGENT_LIST_EnvironmentConfiguration}
     *                                        or just use an individual name
     * @return the DefaultListModel of agents to be started for the specified list
     */
    public DefaultListModel<AgentClassElement4SimStart> getAgentDefaultListModel(
	    DefaultListModel<AgentClassElement4SimStart> newDefaultListModel4AgentStarts, String listName) {
     ...
     * @param mergeListModels indicates if the over all {@link #agentList} has to be
     *                        build new
     * @return true, if the agent name already exists
     */
    ...
    /**
     * Will find a new unique name for an agent, if the suggestion is not already
     * unique.
     *
     ...
     * garanty that the file really exists.
     *
	...
	if (fileToCheckPath.endsWith(USER_MODEL_XML_FileSuffix)) {
	    fileToCheckPath = fileToCheckPath.substring(0,
		    fileToCheckPath.length() - USER_MODEL_XML_FileSuffix.length()) + XML_FileSuffix;
	} else if (fileToCheckPath.endsWith(USER_MODEL_BIN_FileSuffix)) {
	    fileToCheckPath = fileToCheckPath.substring(0,
		    fileToCheckPath.length() - USER_MODEL_BIN_FileSuffix.length()) + XML_FileSuffix;
