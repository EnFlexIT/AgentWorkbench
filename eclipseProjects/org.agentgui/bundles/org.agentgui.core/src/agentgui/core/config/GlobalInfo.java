...
/**
 * This class is for constant values or variables that can be accessed or used
 * application wide.<br>
 * In the Application class the running instance can be accessed by using
 * {@link Application#getGlobalInfo()}.
 * 
...
public class GlobalInfo implements LastSelectedFolderReminder {
	...
	ExecutedOverIDE, ExecutedOverProduct
    }

    /**
     * The Enumeration of possible ExecutionModes. In order to get the current
     * execution mode use the method {@link GlobalInfo#getExecutionMode()} and
     * {@link GlobalInfo#getExecutionModeDescription()}
     * 
    ...
    /**
     * The Enumeration EmbeddedSystemAgentVisualisation indicates if and how
     * visualizations are to be displayed in case that the {@link ExecutionMode} is
     * set to {@link ExecutionMode#DEVICE_SYSTEM} or embedded system agent
     * respectively.
     */
    ...
    /**
     * The Enumeration MtpProtocol describes the usable protocols for the message
     * exchange between agent platforms/container
     */
	...
	try {
		...
		try {
		    thisFile = new File(
			    GlobalInfo.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath());
    ...
    /**
     * Initializes this class by reading the file properties and the current version
     * information.
     */
    ...
    /**
     * Checks if the currently running AWB core bundle is an updated version in
     * comparison to the previously executes version.
     * 
     * @return true, if the current core bundle version is different to the previous
     *         execution
     */
	...
	if (isUpdatedAwbCoreBundle == null) {
	    ...
	    String propsVersionString = this.getStringFromConfiguration(BundleProperties.DEF_AWG_CORE_BUNLDE_VERSION,
		    null);
	    if (propsVersionString == null || propsVersionString.isEmpty() == true
		    || propsVersionString.equals(bundleVersionString) == false) {
	...
	for (int i = 0; i < propertiesKeys.size(); i++) {
	    String property = propertiesKeys.get(i);
     ...
     * {@link ExecutionMode#APPLICATION}).
     * 
    ...
    /**
     * This method can be used in order to set a new application title. For the
     * actual renaming of the Application, use the method in the class
     ...
     * Also the class reference will be validated. If the class could not be found
     * the method will return null.
     * 
     * @return the class reference for the current look and feel or null, if the
    ...
    /**
     * Set the current Look and Feel (LnF) for Java Swing by using its class
     * reference.
     * 
     ...
     * executed
     * 
     * @return - 'IDE', if Agent.GUI is executed out of the IDE, where Agent.GUI is
     *         developed OR<br>
    ...
    /**
     * This method will return a substitute String for the new line String. It is
     * used, for example, in the dictionary, so that it is possible to write text
     * with several lines in a single one.<br>
    ...
    // ----------------------------------------------------
    /**
     * This method returns the base path of the application (e. g. 'C:\Program
     * Files\Agent.Workbench\')
     * 
    ...
    /**
     * Returns the relative path to the properties. For a more sophisticated method
     * use {@link #getPathProperty(boolean)} !
     * 
    ...
    /**
     * This method can be invoked in order to get the path to the property folder
     * 'properties\'.
     * 
     ...
     * Returns the property content provider.
     * 
     * @param pathToProperties the path to properties. May be <code>null</code> in
     *                         case that PropertyContentProvider is already
     *                         initiated.
     * @return the property content provider
     */
    ...
    /**
     * This method will return the concrete path to the property file 'agentgui.ini'
    ...
    /**
     * This method will return the absolute path to the base projects directory
     * ('projects\'). If the path does not exists, the path will be created.
     * 
     ...
     *
     * @param forcePathCreation if true, the path will be created if it not already
     *                          exists
     * @return the path to the project folder
     */
	...
	if (projectsDir.exists() == false) {
	    ...
	    this.createDirectoryIfRequired(filePropProjectsDirectory);
	    System.err.println(
		    "[projects directory] => Switched to default location '" + filePropProjectsDirectory + "'");
    ...
    /**
     * Returns the list of project folders, which are located in the root project
     * folder
     * 
    ...
    /**
     * Defines the local root directory that is used by the file.manager to provide
     * file content. In this folder sources will be stored, which can be loaded to
     * remote containers in order to enable a distributed execution of a MAS. If the
     * folder doesn't exists, it will be created.
     *
     * @param forceDirectoryCreation set true, if the directory should be created
     * @return path to the folder, where possible downloads will be stored
     *         ('/AgentGUI/fmServer[PID]/')
     */
	...
	if (forceDirectoryCreation == true)
	    this.createDirectoryIfRequired(returnPath);
	return returnPath;
    }

    /**
     * This method returns the folder, where download files, coming from an
     * applications (server.client) can be stored locally. If the folder doesn't
     * exists, it will be created.
     *
     * @param forceDirectoryCreation set true, if the directory should be created
     * @return path to the folder, where downloads will be saved
     *         ('/AgentGUI/fmDownload[PID]/')
     */
	...
	if (this.getExecutionEnvironment() == ExecutionEnvironment.ExecutedOverIDE) {
	    ...
	    // --- Executed in the IDE ------------------------------
	    // ------------------------------------------------------
	    String instDirPath = this.getStringFromConfiguration(BundleProperties.DEF_PRODUCT_INSTALLATION_DIRECTORY,
		    null);
	    if (instDirPath == null)
		return null;
		...
		if (checkFile.isDirectory() == false && checkFile.canExecute() == true) {
		    ...
		    boolean isAgentGuiExecutable = checkFileName.startsWith("Agent")
			    && checkFileName.endsWith(".ini") == false && checkFileName.endsWith(".bat") == false;
    ...
    /**
     * This method can be used in order to get the applications executable jar-file
     * that is in fact the equinox launcher.
     * 
    ...
    /**
     * This method can be used in order to get the applications executable jar-file
     * that is in fact the equinox launcher. Set the installation path parameter to
     * <code>null</code> to receive the executable for this application. Set this
     * parameter to a specific one, in order to check any directory for an
     * installation.
     *
     * @param installationPath  an initial installation path that allows to check
     *                          any directory
     * @param statusMessageList define an ArrayList here to receive the errors found
     *                          by the method
     * @return the absolute path to the executable jar file
     */
	...
	try {
		...
		if (this.getExecutionEnvironment() == ExecutionEnvironment.ExecutedOverIDE) {
		    ...
		    instDirPath = this.getStringFromConfiguration(BundleProperties.DEF_PRODUCT_INSTALLATION_DIRECTORY,
			    null);
		...
		this.setErrorMessage(Language.translate("Could not find the installation directory", Language.EN) + " '"
			+ instDirPath + "'.", statusMessageList);
		return null;
	    }
		...
		this.setErrorMessage(Language.translate("Could not find sub directory 'plugins'.", Language.EN)
			+ " (in " + instDir.getAbsolutePath() + ")", statusMessageList);
		return null;
	    }
		...
		@Override
		public boolean accept(File file) {
		    if (file.isDirectory() == false && file.getName().startsWith("org.eclipse.equinox.launcher"))
			return true;
		    return false;
		}
	    });
	    if (searchResult.length == 0) {
		this.setErrorMessage(Language.translate(
			"Could not find the equinox launcher within the plugins-directory of the installed product.",
			Language.EN), statusMessageList);
		return null;
	    }
    ...
    /**
     * This method can be use in order to get the path to one of the dictionary
     * files (Base64: '*.bin' | CSV-version: '*.csv').
     * 
     * @param defaultBase64 if set true, the method will return the path to the
     *                      default Base64-encoded dictionary file, otherwise the
     *                      path to the csv-version of the dictionary
     * 
     * @param absolute      set true if you want to get the full path to this
    ...
    /**
     * Returns the file name of the xml-file, which contains the project
     * configuration (file: 'agentgui.xml')
     * 
    ...
    /**
     * Returns the file name of the XML file that contains the user object of a
     * Project(file: 'agentguiUserObject.xml')
     * 
    ...
    /**
     * Returns the file name of the binary file, which contains the serializable
     * user object of the Project(file: 'agentgui.bin')
     * 
     * @return file name of the project user object binary. (agentgui.bin)
     */
    ...
    /**
     * This method returns the file suffix for Agent.GUI project files ('agui'),
     * which are used for the project exchange within the application
     * 
    ...
    // ---------------------------------------------------------
    /**
     * This method will convert relative Agent.GUI paths to absolute paths. (e. g.
     * './AgentGUI/properties/' will be converted to
     * 'D:/MyWorkspace/AgentGUI/properties')
     * 
    ...
    /**
     * This method can be used in order to set the port on which JADE is running (by
     * default: 1099)
     * 
    ...
    /**
     * This method can be used in order to set the MTP port on which JADE is running
     * (by default: 1099).
     * 
     ...
     * 
     * @param newJadeUrlConfiguration the new {@link JadeUrlConfiguration} for
     *                                master
     */
     ...
     * 
     * @return instance of class 'PlatformJadeConfig', which holds the configuration
     *         of JADE
     ...
     * Returns the bundle properties that are in fact eclipse preferences.
     * 
     * @param isSetPreferencesToGlobal set <code>true</code> in order to directly
     *                                 set the preferences to the {@link GlobalInfo}
     * @return the bundle properties
     */
    ...
    /**
     * Returns the benchmark value of the current execution of Agent.GUI that will
     * be measured at the initial program execution (if required)
     * 
    ...
    /**
     * Here the reminder value can be get, which stores the computer/host name on
     * which Agent.GUI was executed the last time (evaluated due:
     * <code>InetAddress.getLocalHost().getCanonicalHostName())</code>.<br>
     * This can prevent the execution of the benchmark measurement, if the
    ...
    /**
     * This method can be used in order to set the current application language to
     * the property file.
     * 
    ...
    /**
     * Returns the relative logging base path. If not set differently, the method
	...
	String monthDescriptor = new SimpleDateFormat("MM").format(date) + "_"
		+ new SimpleDateFormat("MMM").format(date);
	String logPathByMonth = this.getLoggingBasePath(forcePathCreation) + File.separator + monthDescriptor
		+ File.separator;
	if (forcePathCreation == true) {
    ...
    /**
     * Can be used in order to start the active server mode immediately after the
     * program execution. This applies only if the current execution mode is set to
     * server (server.master or server.slave)
     * 
    ...
    /**
     * This method returns the current setting for the file property '10_AUTOSTART',
     * which is used for whether or not to start the server execution mode
     * immediately after the program execution. This applies only if the current
     * execution mode is set to server (server.master or server.slave)
     * 
    ...
    /**
     * This method can be used in order to remind the last folder in which a file
     * was selected (e. g. while using a JFileChooser)
     * 
	...
	if (knownEnvironmentTypes == null) {
	    ...
	    knownEnvironmentTypes.add(new EnvironmentType(envKey, envDisplayName, envDisplayNameLanguage,
		    envControllerClass, displayAgentClass));
	}
    ...
    /**
     * This method can be used in order to add a tailored environment type (assume
     * for example a 3D-environment model).
     *
     ...
     * 
     * @param deviceServiceAgentVisualisation the new device service agent
     *                                        visualisation
     */
     ...
     * 
     * @param filePropTrustStorePasswordEncrypted the new trust store password
     *                                            encrypted
     */
    ...
    // ------------------------------------------------------------------------------------------------------
    // ---- From here some help methods for container and component handling can be
    // found --- Start ---------
    // ------------------------------------------------------------------------------------------------------
    /**
     ...
     * Returns one of the internal images as ImageIcon, specified by its file name.
     * 
     * @param internalImagePath the internal image path to use (if null, the
     *                          internal image path will be used
     *                          {@link #getPathImageIntern()})
     * @param imageFileName     the image file name
     * @return the internal image icon
     */
	...
	if (internalImagePath == null)
	    internalImagePath = getPathImageIntern();
