 ...
 * 
 * Builds up a tree: cluster-&gt;machine-&gt;JVM-&gt;Container Each clickable
 * node displays details about CPU utilization an thread times. Filter for
 * agents applicable.
 * 
...
public class ThreadInfoStorageTree extends JTree implements ActionListener {
	...
	if ((ae.getSource() == viewSingle) || (ae.getSource() == viewAgentClass)) {
		...
		if (obj.getClass().toString().endsWith("ThreadInfoStorageAgent")) {
			...
			iteratorClass = threadInfoStorage.getMapAgent().get(className).getXYSeriesMap().keySet()
				.iterator();
		    } else {
			...
			iteratorClass = threadInfoStorage.getMapAgentClass().get(className).getXYSeriesMap().keySet()
				.iterator();
		    }
		}
	    }
		...
		new ThreadInfoStorageScrollPane(popupXYSeriesCollectionDelta, popupXYSeriesCollectionTotal, null, true,
			folderNamePrefix + className, null);
	    }
	}
    }
	...
	if (e != null) {
		...
		if (actualElement.isLeaf() == true && key.isEmpty() == false
			&& actualElement.toString().toLowerCase().contains(key.toLowerCase()) == false) {
		    // --- immediate removal invalidates the enumeration ---
		    // actualElement.removeFromParent();
		    // --- remember for later removal ---
		    toDeleteVect.add(actualElement);
		}
		if (actualElement.isLeaf() == true
			&& actualElement.toString().toLowerCase().contains(key.toLowerCase()) == true) {
	...
	// --- filter and update ---
	filteredRoot = (DefaultMutableTreeNode) copyNode(
		(DefaultMutableTreeNode) threadInfoStorage.getModel().getRoot());
	...
	// --- filter and update ---
	((DefaultTreeModel) getModel()).setRoot((DefaultMutableTreeNode) copyNode(threadInfoStorage.getRootNode()));
	...
	DefaultMutableTreeNode tempNode = (DefaultMutableTreeNode) copyNode(
	...
	while (e.hasMoreElements()) {
	    ...
	    if (node.isLeaf() == true) {
		sortNodesDescending((DefaultMutableTreeNode) node.getParent(), (DefaultTreeModel) getModel());
	    }
	}
	...
	while (unsorted) {
		...
		ThreadInfoStorageAgent tiaFirst = (ThreadInfoStorageAgent) ((DefaultMutableTreeNode) node.getChildAt(i))
			.getUserObject();
		ThreadInfoStorageAgent tiaSecond = (ThreadInfoStorageAgent) ((DefaultMutableTreeNode) node
			.getChildAt(i + 1)).getUserObject();

		if (tiaFirst.getXYSeriesMap().get("TOTAL_CPU_SYSTEM_TIME").getMaxY() < tiaSecond.getXYSeriesMap()
			.get("TOTAL_CPU_SYSTEM_TIME").getMaxY()) {
     ...
     * component's <code>addMyMouseListener<code> method. When the myMouse event
	...
	public void mousePressed(MouseEvent e) {
		...
		if (userObject.getClass().toString().endsWith("ThreadInfoStorageAgent")) {
			...
			if (path != null) {
				...
				if (pathBounds != null && pathBounds.contains(e.getX(), e.getY())) {
				    getRightClickMenu().show(ThreadInfoStorageTree.this, pathBounds.x,
			...
			if (threadInfoStorage.getMapAgent().get(nodeName) != null) {
				...
				XYSeries series = threadInfoStorage.getMapAgent().get(nodeName).getXYSeriesMap()
					.get(next);
		...
		if (userObject.getClass().toString().endsWith("ThreadInfoStorageCluster")) {
			...
			while (iterator.hasNext()) {
			    String next = iterator.next();
			    XYSeries series = threadInfoStorage.getMapCluster().get(nodeName).getXYSeriesMap()
				    .get(next);
			    if (next.contains("TOTAL")) {
				popupXYSeriesCollectionTotal.addSeries(series);
			    } else if (next.contains("DELTA")) {
			...
			while (iterator.hasNext()) {
			    String next = iterator.next();
			    XYSeries series = threadInfoStorage.getMapMachine().get(nodeName).getXYSeriesMap()
				    .get(next);
			    if (next.contains("TOTAL")) {
				popupXYSeriesCollectionTotal.addSeries(series);
			    } else if (next.contains("DELTA")) {
			...
			while (iterator.hasNext()) {
			    String next = iterator.next();
			    XYSeries series = threadInfoStorage.getMapContainer().get(nodeName).getXYSeriesMap()
				    .get(next);
			    if (next.contains("TOTAL")) {
				popupXYSeriesCollectionTotal.addSeries(series);
			    } else if (next.contains("DELTA")) {
		...
		// --- create scroll-pane in pop-up window ---
		new ThreadInfoStorageScrollPane(popupXYSeriesCollectionDelta, popupXYSeriesCollectionTotal,
			popupXYSeriesCollectionLoad, true, folderNamePrefix + nodeName, extraObject);
	    }
	}
	...
	public MyTreeCellRenderer() {
	    ...
	    this.agentActiveIcon = new ImageIcon(((GlobalInfo.getInternalImageIcon("AgentGUIGreen.png")).getImage())
		    .getScaledInstance(16, 16, java.awt.Image.SCALE_SMOOTH));
	    this.agentInActiveIcon = new ImageIcon(((GlobalInfo.getInternalImageIcon("AgentGUI.png")).getImage())
		    .getScaledInstance(16, 16, java.awt.Image.SCALE_SMOOTH));
	}
	...
	public Component getTreeCellRendererComponent(JTree tree, Object value, boolean sel, boolean expanded,
		...
		if (nodeObj.getClass().toString().endsWith("ThreadInfoStorageAgent")) {
		    ...
		    setToolTipText("Agent: " + classname[classname.length - 1] + " ["
			    + threadInfoStorage.getNoOfAgentsPerClass().get(tia.getClassName()) + "]");
		}
	    }
	    return this;
	}
