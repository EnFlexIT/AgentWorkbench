 ...
 * Therefore, the the OSHI library is used that is bundles within
 * de.enflexit.oshi
 * 
...
public class LoadMeasureOSHI implements Cloneable {
	...
	double memoryPercentageSwap = this
		.doubleRound(((double) this.getUsedMemorySwap() / (double) this.getTotalMemorySwap() * 100.0));

	if (this.debug) {
	    System.out.println("Total Memory=" + this.getTotalMemory() + " - Available Memory=" + this.getFreeMemory()
		    + " - (" + this.getUsedMemoryPercentage() + " % usage)");
	    System.out.println("Swap Memory=" + this.getTotalMemorySwap() + " - Free Swap Memory="
		    + this.getFreeMemorySwap() + " - (" + memoryPercentageSwap + " % usage)");
	    System.out.println();
	}
    }
	...
	if (processor != null) {
		...
		cpuUsageBetweenTicksOwn = this
			.doubleRound(((double) totalCpuUsageDelta / (double) totalCpuTimeDelta) * 100.0);
	    }
