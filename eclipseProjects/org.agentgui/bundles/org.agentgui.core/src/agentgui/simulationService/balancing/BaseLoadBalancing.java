...
//			e.printStackTrace();
    ...
    /**
     * This method receives the SimulationsServiceHelper to the corresponding local
     * variable.
     */
...
//			e.printStackTrace();
	...
	if (ace4SimStart.getStartArguments() != null) {
	    ...
	    OntologyInstanceViewer oiv = new OntologyInstanceViewer(currProject.getOntologyVisualisationHelper(),
		    currProject.getAgentStartConfiguration(), selectedAgentReference);
	...
	if (agentClassName == null | agentClassName.equalsIgnoreCase("")
		| agentClassName.equalsIgnoreCase(Language.translate("Keine"))) {
	    System.err.println(Language
		    .translate("Agent '" + nickName + "': " + Language.translate("Keine Klasse definiert") + "!"));
	} else {
     ...
     * @return true, if successful
     */
    protected boolean startAgentsOnContainers(
	    Hashtable<Location, ArrayList<AgentClassElement4SimStart>> agentContainerList, boolean verbose) {
	...
	if (agentContainerList != null) {
		...
		for (Iterator<AgentClassElement4SimStart> it = agentContainerList.get(location).iterator(); it
			.hasNext();) {
		    ...
		    boolean success = this.startAgent(agent2Start.getStartAsName(),
			    agent2Start.getAgentClassReference(), startArgs, location);

		    if (success && verbose) {
			System.out
				.println("Agent " + agent2Start.getStartAsName() + " started on " + location.getName());
		    }
		}
	    } // --- end while
	    return true;
     ...
     * {@link #currContainerBenchmarkResults}.
     *
     * @param remoteContainerConfig the remote container configuration out of the
     *                              Project
     * @return the name of the new container
     * 
     * @see #currContainerLoactions
     ...
     * @param filterMainContainer   true, if the Main-Container should be filter out
     *                              of the result
     * 
     * @return the newly started locations
     */
    protected Hashtable<String, Location> startNumberOfRemoteContainer(int numberOfContainer,
	    boolean filterMainContainer, RemoteContainerConfig remoteContainerConfig) {
