...
public abstract class StaticLoadBalancingBase extends BaseLoadBalancing {
     ...
     * 
     * @param agent the {@link LoadExecutionAgent}
     */
    ...
    /**
     * This method will start all agents defined in the agent list of 'Agent-Start'
     ...
     * from the 'Simulation-Setup'.
     * 
     * @param printAgentStart2Console if true, every agent start will be printed in
     *                                the console
     */
	...
	for (Iterator<AgentClassElement4SimStart> iterator = currAgentList.iterator(); iterator.hasNext();) {
	    ...
	    // --- Start the agent -----------------------------------
	    if (debug == true) {
		System.out.println("Start Agent " + counter + ": " + agent2Start.getStartAsName() + " Class: "
			+ agent2Start.getAgentClassReference());
	    }
    ...
    /**
     * This method will start the agent that will show the visualisation of the
     * current environment model. In case of an headless operation of Agent.GUI,
	...
	if (envController != null) {
		...
		currProject.getProjectEditorWindow()
			.setFocus2Tab(Language.translate(ProjectWindowTab.TAB_4_RUNTIME_VISUALIZATION));
	    }
	    // ----------------------------------------------------------------
	}
    }
