...
//				localServiceActuator.setAgentMigrated(aid);
	...
	private class CommandIncomingFilter extends Filter {
		...
		public boolean accept(VerticalCommand cmd) {
			...
			String cmdName = cmd.getName();
//			System.out.println( "<= Incomming " + cmdName + " - " + cmd.getService() + " class: " + cmd.getClass());
			
			if (myMainContainer != null) {
				// --- We're in the main container ------------------
				if (cmdName.equals(AgentManagementSlice.SHUTDOWN_PLATFORM)==true) {
					// --- Stop all simulating agents ---------------
					SimulationService.this.getLocalSlice().stopSimulationAgents();
					
				} else if (cmdName.equals(AgentManagementSlice.KILL_CONTAINER)==true) {
					// --- Stop all simulating agents ---------
					SimulationService.this.getLocalSlice().stopSimulationAgents();
