...
public abstract class SimulationServiceBehaviour extends Behaviour implements ServiceSensorInterface {
    ...
    /**
     * Instantiates a new SimulationServiceBehaviour as an active agent, which means
     * that the current agent is asked to react on notifications or environment
     * changes.
     * 
    ...
    /**
     * Instantiates a new simulation agent as a passive agent. The agent will just
     * listening to changes in the environment model but it is not expected, that it
     * will react on it. This is used for example for agents, which are displaying
	...
	try {
	    SimulationServiceHelper simHelper = (SimulationServiceHelper) this.myAgent
		    .getHelper(SimulationService.NAME);
    ...
    /**
     * This method will be used by the ServiceActuator (class) to inform this agent
     * about its new migration location.
     *
    ...
    /**
     * This method will be used by the {@link ServiceActuator} to inform this agent
     * about changes in the environment. It can be either used to do this
    ...
    /**
     * This method is internally called if a stimulus from the outside reached this
     * agent.
     */
    ...
    /**
     * This method is called if a stimulus from the outside reached this agent. It
     * can be overwritten in the child class to act on environment changes.
     */
    public abstract void onEnvironmentStimulus();

    /**
     * This method sets the answer respectively the change of a single simulation
     * agent back to the central simulation manager.
     *
     * @param myNextState the next state of this agent in the next instance of the
     *                    environment model
     */
    ...
    /**
     * This method can be used to transfer any kind of information to the Manager of
	...
	boolean send = false;
	EnvironmentNotification myNotification = new EnvironmentNotification(this.myAgent.getAID(), false,
		notification);
	try {
	    SimulationServiceHelper simHelper = (SimulationServiceHelper) this.myAgent
		    .getHelper(SimulationService.NAME);
    ...
    /**
     * This method can be used to transfer any kind of information to one member of
     * the current environment model.
     *
	...
	try {
	    EnvironmentNotification notification = new EnvironmentNotification(getAID(), true,
    ...
    /**
     * This method can be invoked from the simulation service, if a notification for
     * the manager has to be delivered.
     *
