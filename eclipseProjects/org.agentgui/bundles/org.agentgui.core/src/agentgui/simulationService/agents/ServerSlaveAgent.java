...
public class ServerSlaveAgent extends Agent {
	...
	protected void setup() {
		...
		try {
			...
			if (startConfig!=null) {
				System.out.println("[" + this.getName() + "] Found remote container start configuration.");
				JadeRemoteStartConfiguration.deleteRemoteStartConfiguration();
				this.startRemoteContainer(startConfig);
			} else {
				System.out.println("[" + this.getName() + "] No start configuration for a remote container was found.");
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}
