...
/**
 * The Class LogFileWriter enables to redirect console output to a specified log
 * file.
 * 
...
public class LogFileWriter {
    ...
    /**
     * Instantiates a new log file writer that uses the log directory for the
     * logging file.
     */
    public LogFileWriter() {
	this.initialize();
	this.appendText(new PrintStreamListenerOutput(PrintStreamListenerType.SystemOutput,
		Application.getGlobalInfo().getVersionInfo().getVersionInfo()));
    }
	...
	if (logFileName == null) {
	    System.err.println(Application.getGlobalInfo().getApplicationTitle()
		    + "-Logging: Log file name was null. Thus, the log file writer will not be started.");
	} else {
	...
	try {
	    ...
	    if (pslOutput.getLineOutput() == null)
		return;
    ...
    /**
     * Deletes old log files. Based on the current date and its corresponding day
     * prefix, the files that are older than {@link #CONSERVATION_PERIOD} will be
     * removed out of the corresponding month directory
     *
	...
	if (logDirectory.exists() == true) {
		...
		try {
			...
			System.err.println(
				"Agent.GUI-Logging: Log file '" + oldLog.getAbsolutePath() + "' could not be removed.");
		    } else {
			System.out
				.println("Agent.GUI-Logging: Removed old log file '" + oldLog.getAbsolutePath() + "'.");
		    }
