...
public abstract class DynFormBase {
	...
	private DynTreeViewer dtv;
	/** True, if no class has to be visualized on the DynForm */
	...
	private HashMap<DefaultMutableTreeNode, OntologyClassWidget> ontologyClassWidget;
	/** Reminder Hash that stores the tree node of a given {@link DynType} */
	private HashMap<DynType, DefaultMutableTreeNode> treeNodesByDynType;
	...
	private Object getNewClassInstance(String className) {
		...
		try {
			...
			if (clazz.isInterface()==false) {
				// --- OntologyBeanGenerator for Protege 3.3.1 ---------------- 
				obj = BaseClassLoadServiceUtility.newInstance(className);	
				...
				obj = BaseClassLoadServiceUtility.newInstance(defaultClass);
			}
