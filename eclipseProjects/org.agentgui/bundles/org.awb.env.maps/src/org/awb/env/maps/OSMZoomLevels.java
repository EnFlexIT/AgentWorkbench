...
/**
 * The singleton class OSMZoomLevels holds all available {@link ZoomLevel} that
 * correspond to the zoom level of OpenStreetMap.
 *
...
public class OSMZoomLevels {
	...
	// --- $zoomstufen = array(156412, 78206, 39103, 19551, 9776, 4888, 2444, 1222,
...
//		this.getZoomLevelList().add(new ZoomLevel(19, 0.298));
    ...
    /**
     * Checks if the current resolution needs to be recalculated based on the new
     * latitude coordinate.
     *
    ...
    /**
     * Returns the closest {@link ZoomLevel} for the specified map resolution in
     * m/pixel.
     *
	...
	if (Double.isNaN(resolution) == true)
	    return zlFirst;
	if (resolution >= zlFirst.getResolution())
	    return zlFirst;
	if (resolution <= zlLast.getResolution())
	    return zlLast;
    ...
    /**
     * Rounds a double value to the given precision. For example: <br>
    ...
    /**
     * The Class ZoomLevel describes the zoom level relationship between OSM zoom
     * level resolution and the scaling of the Jung graph.
     *
     * @author Christian Derksen - SOFTEC - ICB - University of Duisburg-Essen
     */
	...
	public boolean equals(Object compObj) {
	    if (!(compObj instanceof ZoomLevel))
		return false;
	    if (compObj == this)
		return true;
	    ZoomLevel zlComp = (ZoomLevel) compObj;
	    if (zlComp.getOSMZoomLevel() != this.getOSMZoomLevel())
		return false;
	    if (zlComp.getResolution() != this.getResolution())
		return false;
	    return true;
	}
	...
	public String toString() {
	    return "OSM-Zoom level: " + this.getOSMZoomLevel() + ", JXMapViewer-Zoom level: "
		    + this.getJXMapViewerZoomLevel() + ", Map-Resolution: " + this.getResolution()
		    + " m/pixel, Jung-Scaling: " + this.getJungScaling();
	}
    }
