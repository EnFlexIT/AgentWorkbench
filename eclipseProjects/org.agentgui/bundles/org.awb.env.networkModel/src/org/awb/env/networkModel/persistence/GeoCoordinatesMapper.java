...
/**
 * This class provides mapping functionality from geo coordinates to display
 * coordinates (0:0)
 * 
...
public class GeoCoordinatesMapper {
    ...
    // --- The target range of display coordinates (upper bounds, the actual range
    // depends on the ration of the original data range)
    private int targetRangeMaxX;
    private int targetRangeMaxY;

    // --- These offsets can be used to shift the mapped coordinates, if the
    // smallest coordinates should not start at 0:0
    ...
    // --- The conversion factor will be calculated, based on the ration between
    // original and target range
     ...
     */
    public GeoCoordinatesMapper(double geoCoordinateMinX, double geoCoordinateMaxX, double geoCoordinateMinY,
	    double geoCoordinateMaxY, int targetRangeMaxX, int targetRangeMaxY) {
     ...
     * 
     * @param targetRangeMaxX the new upper bound for the target range's x
     *                        coordinate
     */
     ...
     * 
     * @param targetRangeMaxY the new upper bound for the target range's y
     *                        coordinate
     */
    ...
    /**
     * Calculates conversion factor for mapping coordinates, and the actual target
     * range, that is defined by the user-specified upper bounds and the ration of
	...
	double displayCoordinateX = (geoCoordinateX - this.geoCoordinateMinX) * this.getConversionFactor()
		+ this.getOffsetX();
	double displayCoordinateY = (geoCoordinateY - this.geoCoordinateMinY) * this.getConversionFactor()
		+ this.getOffsetY();
	// --- Invert, if specified
