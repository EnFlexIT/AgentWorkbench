 ...
 * {@link NetworkModel} if the {@link LayoutSettings} are set to a 'geographical
 * layout'.
 * 
 * @author Christian Derksen - DAWIS - ICB - University of Duisburg - Essen
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "MapSettings", propOrder = { "utmLongitudeZone", "utmLatitudeZone", "mapScale", "mapServiceName",
	"mapTileTransparency" })
public class MapSettings implements Serializable, Cloneable {
	...
	s1x(1, "1 x"), s2x(2, "2 x"), s3x(3, "3 x"), s4x(4, "4 x"), s5x(5, "5 x"), s10x(10, "10 x"), s15x(15, "15 x"),
	s20x(20, "20 x"), s25x(25, "25 x");
	...
	if (compObj == this)
	    return true;
	if (!(compObj instanceof MapSettings))
	    return false;

	MapSettings msComp = (MapSettings) compObj;

	if (msComp.getUTMLongitudeZone() != this.getUTMLongitudeZone())
	    return false;
	if (msComp.getUTMLatitudeZone().equals(this.getUTMLatitudeZone()) == false)
	    return false;

	if (msComp.getMapServiceName().equals(this.getMapServiceName()) == false)
	    return false;
	if (msComp.getMapScale().equals(this.getMapScale()) == false)
	    return false;
	if (msComp.getMapTileTransparency() != this.getMapTileTransparency())
	    return false;
    ...
    // ----------------------------------------------------------------------------------
    /**
     * Return the current {@link MapService}. if <code>Null</code>, it will be tried
     * to load the currently configured MapsService from the list of available
     * services
     * 
    ...
    /**
     * Returns the {@link MapService} specified by the map service name or
     * <code>Null</code>.
     *
    ...
    /**
     * Returns the MapService names as sorted list, where the first element is the
     * indicator for not using a MapService.
     *
