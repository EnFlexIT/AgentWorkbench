...
public class GraphEnvironmentEdgeRenderer extends BasicEdgeRenderer<GraphNode, GraphEdge> {
     ...
     * 
     * @param visViewer the {@link BasicGraphGuiVisViewer}
     */
     ...
     * 
     * @see
     * edu.uci.ics.jung.visualization.renderers.BasicEdgeRenderer#drawSimpleEdge(edu
     * .uci.ics.jung.visualization.RenderContext,
     * edu.uci.ics.jung.algorithms.layout.Layout, java.lang.Object)
     */
    @Override
    protected void drawSimpleEdge(RenderContext<GraphNode, GraphEdge> rc, Layout<GraphNode, GraphEdge> layout,
	    GraphEdge edge) {
    ...
    /**
     * Draws the specified simple edge and represents basically the original but
     * adjusted super method.
     *
     ...
     */
    private void drawSimpleEdgeAdjusted(RenderContext<GraphNode, GraphEdge> rc, Layout<GraphNode, GraphEdge> layout,
	    GraphEdge edge, boolean isPaintMarker) {
	...
	boolean isOrthogonalConfigurableLine = (ls
		.getEdgeShape() == org.awb.env.networkModel.settings.LayoutSettings.EdgeShape.ConfigurableLine
		&& edge.getEdgeShapeConfiguration() != null
		&& edge.getEdgeShapeConfiguration() instanceof OrthogonalConfiguration);
	boolean isOrthogonal = rc.getEdgeShapeTransformer() instanceof EdgeShape.Orthogonal
		|| isOrthogonalConfigurableLine;
	...
	if (isLoop == true) {
	    ...
	    edgeShape = getGeneralPathForOrthogonalConnection(rc, layout, edge, x1, y1, x2, y2,
		    ls.getCoordinateSystemXDirection());
	...
	// ----------------------------------------------------------------------------------------
	// => ADJUSTED FORM
	edgeShape = getTransformedShape(edgeShape, this.visViewer.getCoordinateSystemPositionTransformer(), xform, rc,
		v1, v2, isOrthogonal);
	// ----------------------------------------------------------------------------------------
	...
	if (edgeHit == true) {
	    ...
	    if (rc.getEdgeArrowPredicate()
		    .apply(Context.<Graph<GraphNode, GraphEdge>, GraphEdge>getInstance(graph, edge))) {
		...
		arrowHit = rc.getMultiLayerTransformer().getTransformer(Layer.VIEW).transform(destVertexShape)
		...
		if (graph.getEdgeType(edge) == EdgeType.UNDIRECTED) {
		    ...
		    arrowHit = rc.getMultiLayerTransformer().getTransformer(Layer.VIEW).transform(vertexShape)
			    .intersects(deviceRectangle);

		    if (arrowHit) {
			AffineTransform at = edgeArrowRenderingSupport.getReverseArrowTransform(rc, edgeShape,
				vertexShape, !isLoop);
			if (at == null)
			    return;
			Shape arrow = rc.getEdgeArrowTransformer()
     ...
     */
    public static Shape getTransformedShape(Shape shape, TransformerForGraphNodePosition csTransformer,
	    AffineTransform affineTransform, RenderContext<GraphNode, GraphEdge> rc, GraphNode startNode,
	    GraphNode endNode, boolean isOrthogonal) {
	...
	if (isOrthogonal == false && shape instanceof GeneralPath) {
		...
		if (!(xCoord == 0.0 && yCoord == 0.0 || xCoord == 1.0 && yCoord == 0.0)) {
		    Point2D scaledPoint = getIntermediatePointTransformer()
			    .transformToGraphCoordinate(new Point2D.Double(xCoord, yCoord), startNode, endNode);
	    ...
	    Point2D scaledControlPoint = getIntermediatePointTransformer()
		    .transformToGraphCoordinate(quadCurve.getCtrlPt(), startNode, endNode);
     ...
     */
    public static GeneralPath getGeneralPathForOrthogonalConnection(RenderContext<GraphNode, GraphEdge> rc,
	    Layout<GraphNode, GraphEdge> layout, GraphEdge edge, float x1, float y1, float x2, float y2,
	    CoordinateSystemXDirection xDirection) {
	...
	if (rc.getEdgeShapeTransformer() instanceof ParallelEdgeShapeTransformer) {
	    ...
	    EdgeIndexFunction<GraphNode, GraphEdge> peif = ((ParallelEdgeShapeTransformer<GraphNode, GraphEdge>) rc
	...
	if (edge.getEdgeShapeConfiguration() != null
		&& edge.getEdgeShapeConfiguration() instanceof OrthogonalConfiguration) {
