...
public class MapRendererSettings implements Serializable {
    ...
    /**
     * Initialize all fields derived from the current visualization viewer and its
     * affine transformer to map coordinates from layout to visualization.
     *
     * @param visViewer the vis viewer
     */
	...
	UTMCoordinate topLeftCoordinate = this.getUTMCoordinate(cspt, at, new Point2D.Double(0, 0),
		isPrintTransformation);
	UTMCoordinate topRightCoordinate = this.getUTMCoordinate(cspt, at, new Point2D.Double(vvDim.getWidth(), 0),
		isPrintTransformation);
	UTMCoordinate bottomLeftCoordinate = this.getUTMCoordinate(cspt, at, new Point2D.Double(0, vvDim.getHeight()),
		isPrintTransformation);
	UTMCoordinate bottomRightCoordinate = this.getUTMCoordinate(cspt, at,
		new Point2D.Double(vvDim.getWidth(), vvDim.getHeight()), isPrintTransformation);
	...
	this.setCenterPostion(
		this.getWGS84LatLngCoordinate(cspt, at, new Point2D.Double(centerX, centerY), isPrintTransformation));
	...
	AffineTransform lat = visViewer.getRenderContext().getMultiLayerTransformer().getTransformer(Layer.LAYOUT)
		.getTransform();
	AffineTransform vat = visViewer.getRenderContext().getMultiLayerTransformer().getTransformer(Layer.VIEW)
		.getTransform();
    ...
    /**
     * Returns a WGS 84 coordinate from the specified visual point in the
     * application.
     *
     * @param cspTransformer        the coordinate system position transformer
     * @param at                    the AffineTransform to transform to Jung
     *                              coordinates
     * @param point2D               the point 2 D
     * @param isPrintTransformation the indicator to print the transformation of the
     *                              coordinates
     * @return the WGS84LatLngCoordinate
     */
    private WGS84LatLngCoordinate getWGS84LatLngCoordinate(
	    TransformerForGraphNodePosition<GraphNode, GraphEdge> cspTransformer, AffineTransform at, Point2D point2D,
	    boolean isPrintTransformation) {
     ...
     */
    private UTMCoordinate getUTMCoordinate(TransformerForGraphNodePosition<GraphNode, GraphEdge> cspTransformer,
	    AffineTransform at, Point2D point2D, boolean isPrintTransformation) {
	...
	try {
	    ...
	    Point2D pointJung = at.inverseTransform(point2D, null);
	    Point2D pointCoSy = cspTransformer.inverseTransform(pointJung);
	    utm = new UTMCoordinate(ms.getUTMLongitudeZone(), ms.getUTMLatitudeZone(), pointCoSy.getX(),
		    pointCoSy.getY());
	    if (isPrintTransformation == true) {
		System.out.println("Pos. Screen   " + point2D.getX() + ", " + point2D.getY() + "");
		System.out.println("Pos. Jung     " + pointJung.getX() + ", " + pointJung.getY() + "");
