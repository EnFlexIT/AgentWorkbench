...
/**
 * Abstract superclass for CSV-based network model importer implementations,
...
public abstract class AbstractNetworkModelCsvImporter extends AbstractNetworkModelFileImporter {
     ...
     * 
     * @see
     * org.awb.env.networkModel.controller.NetworkModelFileImporter#cleanupImporter(
     * )
     */
    @Override
	...
	if (networkModel == null) {
	    ...
	    networkModel
		    .setGeneralGraphSettings4MAS(this.graphController.getNetworkModel().getGeneralGraphSettings4MAS());
	}
	return networkModel;
    }

    /**
     * Reads the data from the CSV files and stores it in separate
     * {@link CsvDataController}s. If not all files could be found an error will be
     * shown.
     * 
    ...
    /**
     * Reads the data from the CSV files and stores it in separate
     * {@link CsvDataController}s.
     *
     * @param graphFile                 the file to read
     * @param isAllowIncompleteFileList the indicator if an incomplete list of file
     *                                  is allowed or not
     * @return true, if successful
     * 
     * @see #getCsvDataController()
	...
	if (isAllowIncompleteFileList == false
		&& this.getCsvDataController().size() < this.getValidFileNames().size()) {
	    this.errTitle = "Incomplete Import:";
	    this.errMessage = "Missing standard csv files within the selection (found "
		    + this.getCsvDataController().size() + " of " + this.getValidFileNames().size()
		    + " required files)!";
	    successful = false;
	}
	return successful;
	...
	if ((this.errTitle != null && this.errTitle.isEmpty() == false)
		|| this.errMessage != null && this.errMessage.isEmpty() == false) {
	    JOptionPane.showMessageDialog(this.graphController.getGraphEnvironmentControllerGUI(), this.errMessage,
		    this.errTitle, JOptionPane.ERROR_MESSAGE);
	}
    }
