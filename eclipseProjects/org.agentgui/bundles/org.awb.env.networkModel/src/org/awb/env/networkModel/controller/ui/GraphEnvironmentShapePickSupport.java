...
/**
 * The Class GraphEnvironmentShapePickSupport extends the Jung ShapePickSupport
 * and overwrites {@link #getEdge(Layout, double, double)} method to capture
 * orthogonal and polyline edges (In the original internal method
 * 'getTransformedEdgeShape()' lacks of these two edge types).
 * 
...
public class GraphEnvironmentShapePickSupport extends ShapePickSupport<GraphNode, GraphEdge> {
     ...
     * 
     * @see
     * edu.uci.ics.jung.visualization.picking.ShapePickSupport#getEdge(edu.uci.ics.
     * jung.algorithms.layout.Layout, double, double)
     */
    @Override
    ...
    /**
     * Retrieves the shape template for <code>e</code> and transforms it according
     * to the positions of its endpoints in <code>layout</code>.
     * 
     * @param layout the <code>Layout</code> which specifies <code>e</code>'s
     *               endpoints' positions
	...
	boolean isOrthogonal = vv.getRenderContext().getEdgeShapeTransformer() instanceof EdgeShape.Orthogonal
	...
	if (isLoop == true) {
	    ...
	    LayoutSettings ls = this.getVisualizationViewer().getCoordinateSystemPositionTransformer()
		    .getLayoutSettings();
	    edgeShape = GraphEnvironmentEdgeRenderer.getGeneralPathForOrthogonalConnection(vv.getRenderContext(),
		    layout, edge, x1, y1, x2, y2, ls.getCoordinateSystemXDirection());
	...
	// ----------------------------------------------------------------------------------------
	// => ADJUSTED FORM
	edgeShape = GraphEnvironmentEdgeRenderer.getTransformedShape(edgeShape,
		this.getVisualizationViewer().getCoordinateSystemPositionTransformer(), xform, vv.getRenderContext(),
		v1, v2, isOrthogonal);
	// ----------------------------------------------------------------------------------------
	return edgeShape;
    }
