...
/**
 * A factory for creating GraphNodePosition objects that extend the class
 * {@link AbstractCoordinate}.
 */
public class GraphNodePositionFactory {
	...
	SimpleXY, WGS84, UTM, OSGB
    }

    private static NetworkModel networkModelLoading;

    // --------------------------------------------------------------------------------------------
    // --- From here some help methods to access the current project ----------
    // Start -------------
    // --------------------------------------------------------------------------------------------
    /**
     * Returns the current {@link GraphEnvironmentController} if accessible in the
     * current project.
     * 
    ...
    /**
     * Returns the current {@link NetworkModel} if accessible in the current
     * project.
     * 
    ...
    /**
     * Converts a specified source coordinate to the specified destination
     * coordinate type - if this is possible in general.<br>
     * For example: a simple {@link GraphNodePosition} is not convertible into a
     * geographical coordinate, while a WGS84 coordinate can be converted into an
     * UTM coordinate and vice versa.<br>
     * <br>
     ...
     * @return the converted coordinate, the original coordinate or
     *         <code>null</code>
     */
    public static AbstractCoordinate convertToCoordinate(AbstractCoordinate sourceCoordinate,
	    CoordinateType destinCoordType) {

	// --- Fast exit ? ----------------------------------------------------
	if (sourceCoordinate == null || destinCoordType == null)
	    return null;

	// --- Unknown or equal coordinate type -------------------------------
	CoordinateType sourceCoordType = getCoordinateType(sourceCoordinate);
	if (sourceCoordType == null)
	    return null;
	if (sourceCoordType == destinCoordType)
    ...
    /**
     * Converts the specified Point2D into a coordinate that is - depending on the
     * current {@link LayoutSettings} in the NetworkModel - a
     * {@link GraphNodePosition} or a geographical coordinate like a
     * {@link WGS84LatLngCoordinate} or an {@link UTMCoordinate}.
     *
    ...
    /**
     * Converts the specified Point2D into a coordinate for the specified layoutID
     * and it's corresponding {@link LayoutSettings}. The instance will be of type
     * {@link GraphNodePosition} or a geographical coordinate like a
     * {@link WGS84LatLngCoordinate} or an {@link UTMCoordinate}.
     *
     * @param layoutID the layout ID
	...
	if (layoutID == null || layoutID.isEmpty() == true)
	    return null;
	...
	// --- Quick exit? ------------------------------------------
	if (coString == null || coString.isEmpty() == true)
	    return null;
	...
	if (coordinate instanceof WGS84LatLngCoordinate)
	    return CoordinateType.WGS84;
	if (coordinate instanceof UTMCoordinate)
	    return CoordinateType.UTM;
	if (coordinate instanceof OSGBCoordinate)
	    return CoordinateType.OSGB;
	if (coordinate instanceof GraphNodePosition)
	    return CoordinateType.SimpleXY;
	return null;
    }

    // --------------------------------------------------------------------------------------------
    // --- From here, the old style conversion from string to Point2D and vice versa
    // --- Start ----
    // --------------------------------------------------------------------------------------------
    /**
