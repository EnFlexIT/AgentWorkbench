...
public class SSLJettyConfiguration {
	...
	private static File createKeyStore(String serverName, char[] password) {
		...
		try {
			...
			X509Certificate cert = SelfSignedCertGenerator.generate(keyPair, "SHA256withRSA", DEFAULT_DOMAIN, 730);
			Certificate[] certArray = {cert};
			
			// --- Define destination file ------------------------------------
			keyStoreFile = new File(BundleHelper.getPathProperties() + serverName + "." + DEFAULT_KEYSTORE_FILE_PREFIX);
