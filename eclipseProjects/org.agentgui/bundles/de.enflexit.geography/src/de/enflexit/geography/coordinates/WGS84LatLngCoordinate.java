...
public class WGS84LatLngCoordinate extends AbstractGeoCoordinate {
	...
	public UTMCoordinate getUTMCoordinate() {
		...
		String UTMZone = getUTMLatitudeZoneLetter(latitude);
		...
		double M = a * ((1 - eSquared / 4 - 3 * eSquared * eSquared / 64 - 5 * eSquared * eSquared * eSquared / 256)
				* latitudeRad
				...
				+ (15 * eSquared * eSquared / 256 + 45 * eSquared * eSquared * eSquared / 1024)
						* Math.sin(4 * latitudeRad)
				- (35 * eSquared * eSquared * eSquared / 3072) * Math.sin(6 * latitudeRad));

		double UTMEasting = (UTM_F0 * n * (A + (1 - t + c) * Math.pow(A, 3.0) / 6
				+ (5 - 18 * t + t * t + 72 * c - 58 * ePrimeSquared) * Math.pow(A, 5.0) / 120) + 500000.0);

		double UTMNorthing = (UTM_F0 * (M
				+ n * Math.tan(latitudeRad) * (A * A / 2 + (5 - t + (9 * c) + (4 * c * c)) * Math.pow(A, 4.0) / 24
				+ (61 - (58 * t) + (t * t) + (600 * c) - (330 * ePrimeSquared)) * Math.pow(A, 6.0) / 720)));
		...
		return new UTMCoordinate(UTMEasting, UTMNorthing, UTMZone, longitudeZone);
	}
	 ...
	 *
	 * @param latitude the latitude
	 * @return the UTM latitude zone letter
	 */
	private String getUTMLatitudeZoneLetter(double latitude) {
