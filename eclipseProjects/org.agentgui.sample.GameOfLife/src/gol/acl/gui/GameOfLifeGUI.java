...
import gol.SimService.agents.SimulationManager;
...
public class GameOfLifeGUI extends JInternalFrame implements ActionListener {
	...
	public JCheckBox visualUpdate = new JCheckBox("Update View");
	...
	public GameOfLifeGUI(int nbRow, int nbCol, Agent simulationManagerAgent) {
		...
		// ------ coordinates of number of gol.SimService.agents -------------------------------------------
		this.nbRow = nbRow;
		this.nbCol = nbCol;
		...
		// ---- add the checkbox for the visual update and the generation label ------------
		JPanel centerPanel = new JPanel(new GridLayout(1,2));
		visualUpdate.setSelected(true);
		centerPanel.add(visualUpdate);
		generationLabel.setHorizontalAlignment(SwingConstants.CENTER);
		generationLabel.setSize(generationLabel.getWidth(), 200);
		centerPanel.add(generationLabel);
	...
	public synchronized void actionPerformed(ActionEvent event) {
		...
		if(object == bClear) {
			...
			if (controllerAgent instanceof SimulationManager) {
				((SimulationManager) controllerAgent).setRunSimulation(false);	
			} else {
		...
		if(object == bStart) {
			...
			if (controllerAgent instanceof SimulationManager) {
				((SimulationManager) controllerAgent).setRunSimulation(true);	
			} else {
				controllerAgent.doActivate();
			}
			return;
