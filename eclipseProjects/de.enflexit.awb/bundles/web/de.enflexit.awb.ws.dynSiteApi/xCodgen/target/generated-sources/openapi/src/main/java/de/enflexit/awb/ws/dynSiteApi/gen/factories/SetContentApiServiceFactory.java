package de.enflexit.awb.ws.dynSiteApi.gen.factories;

import de.enflexit.awb.ws.dynSiteApi.gen.SetContentApiService;
import de.enflexit.awb.ws.dynSiteApi.gen.impl.SetContentApiServiceImpl;

@jakarta.annotation.Generated(value = "org.openapitools.codegen.languages.JavaJerseyServerCodegen", date = "2025-07-09T11:20:39.473761300+02:00[Europe/Berlin]", comments = "Generator version: 7.6.0")
public class SetContentApiServiceFactory {
    private static final SetContentApiService service = new SetContentApiServiceImpl();

    public static SetContentApiService getSetContentApi() {
        return service;
    }
}
