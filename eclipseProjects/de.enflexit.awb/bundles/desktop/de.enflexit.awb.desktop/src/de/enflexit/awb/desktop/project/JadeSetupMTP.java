...
import de.enflexit.awb.baseUI.components.HttpsConfigWindow;
...
//	private boolean pauseDocumentListener;
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (trigger==this.getJCheckBoxSkipUserRequestForJadeStart()) {
			...
			// --- Open Dialog ------------------
			JadeSetupNewPort newPort = new JadeSetupNewPort((MainWindow)Application.getMainWindow(), currProject.getProjectName(), true, this.currProject.getJadeConfiguration().getLocalPort(), jTextFieldDefaultPort.getLocationOnScreen());
			...
			// --- Set MTP port ---------------------------
			JadeSetupNewPort newPort = new JadeSetupNewPort((MainWindow)Application.getMainWindow(), this.currProject.getProjectName(), true, this.currProject.getJadeConfiguration().getLocalPortMTP(), this.getJTextFieldDefaultPortMTP().getLocationOnScreen());
			newPort.setVisible(true);
			// === Go ahead =====================
			if (newPort.isCanceled() == false) {
			...
			if (this.currProject.getJadeConfiguration().getKeyStoreFile()!=null || this.currProject.getJadeConfiguration().getTrustStoreFile()!=null) {
				...
				int userAnswer = JOptionPane.showConfirmDialog((MainWindow)Application.getMainWindow(), message, title, JOptionPane.YES_NO_OPTION);
	...
	private void editHTTPSsettings(MtpProtocol configuredMtpProtocol) {
		...
		if (keyStoreFilePath==null || trustStoreFilePath==null || new File(keyStoreFilePath).exists()==false || new File(trustStoreFilePath).exists()==false) {
			...
			HttpsConfigWindow httpsConfigWindow = new HttpsConfigWindow((MainWindow)Application.getMainWindow());
			...
			// --- Open the HttpsConfigWindow ----------------------------------
			HttpsConfigWindow httpsConfigWindow = new HttpsConfigWindow((MainWindow)Application.getMainWindow(), keyStoreFile, keyStorePassword, trustStoreFile, trustStorePassword);
			httpsConfigWindow.setPreferredDirector(this.currProject.getProjectSecurityFolderFullPath());
			httpsConfigWindow.setVisible(true);
			// --- Wait for the user -------------------------------------------
			if (httpsConfigWindow.isCanceled()==false) {
