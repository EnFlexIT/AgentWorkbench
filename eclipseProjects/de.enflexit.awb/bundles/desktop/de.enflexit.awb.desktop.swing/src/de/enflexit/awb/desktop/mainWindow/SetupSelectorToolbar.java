...
public class SetupSelectorToolbar implements ActionListener {
	...
	private void initialize() {
		...
		this.mainWindowToolBar.add(this.getJButtonSetupNew(), AwbMainWindowToolBarGroup.SetupHandling);
		this.mainWindowToolBar.add(this.getJButtonSetupDelete(), AwbMainWindowToolBarGroup.SetupHandling);
		this.mainWindowToolBar.add(new JToolBar.Separator(), AwbMainWindowToolBarGroup.SetupHandling);

		this.mainWindowToolBar.add(this.getJLabel4SetupSelection(), AwbMainWindowToolBarGroup.SetupHandling);
		this.mainWindowToolBar.add(this.getJComboBoxSetupSelector(), AwbMainWindowToolBarGroup.SetupHandling);
		
		this.mainWindowToolBar.add(this.getJButtonSetupRename(), AwbMainWindowToolBarGroup.SetupHandling);
		this.mainWindowToolBar.add(this.getJButtonSetupCopy(), AwbMainWindowToolBarGroup.SetupHandling);
	...
	public boolean isEnabled() {
		return this.jLabelSetupSelector.isEnabled() && this.getJComboBoxSetupSelector().isEnabled();
	}
	...
	private String setupAskUser4SetupName(SetupAction setupAction, String suggestion) {
		...
		while (inputIsOk==false) {
			...
			if (suggestion== null) {
				input = (String) JOptionPane.showInputDialog(this.mainWindow, msg, head, JOptionPane.OK_CANCEL_OPTION);	
			} else {
				input = (String) JOptionPane.showInputDialog(this.mainWindow, msg, head, JOptionPane.OK_CANCEL_OPTION, null, null, suggestion);	
			}
			...
			if (newFileName.length() < 8) {
				...
				JOptionPane.showMessageDialog(this.mainWindow, msg, head, JOptionPane.NO_OPTION);
			} else {
				inputIsOk = true;
			}
	...
	private void setupRemove() {
		...
		int input = JOptionPane.showConfirmDialog(this.mainWindow, msg, head, JOptionPane.YES_NO_OPTION);
