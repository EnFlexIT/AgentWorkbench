...
import de.enflexit.awb.desktop.dialogs.AgentSelector;
...
public class StartSetup extends JPanel implements Observer, ActionListener {
    ...
    private DefaultListModel<AgentClassElement4SimStart> listModelAgentStart;

    private JSplitPane jSplitPane;
    private JPanel jPanelRightSplit;
    private JPanel jPanelLeftSplit;

    private JPanel jPanelButtons;
    private JPanel jPanelRight;
    private JPanel jPanelOntoloInstView;
    private OntologyInstanceViewer currOntoInstViewer;

    private JScrollPane jScrollPaneStartList;

    private JList<AgentClassElement4SimStart> jListStartList;
    private JButton jButtonAgentAdd;
    private JButton jButtonAgentRemove;
    private JButton jButtonMoveUp;
    private JButton jButtonMoveDown;

    private JTextField jTextFieldStartAs;
    private JButton jButtonStartOK;
    private JLabel jLabelHeader;

    private JLabel jLabelStartList;
    private JComboBox<String> jComboBoxStartLists;
    ...
    /**
     * Returns the list model to locally work on for the agent start.
     * @return the list model agent start
     */
    private DefaultListModel<AgentClassElement4SimStart> getListModelAgentStart() {
    	if (listModelAgentStart==null) {
    		listModelAgentStart = new DefaultListModel<>();
    	}
		return listModelAgentStart;
	}
    /**
     * Sets the list model agent start.
     * @param newListModelAgentStart the new list model agent start
     */
    private void setListModelAgentStart(DefaultListModel<AgentClassElement4SimStart> newListModelAgentStart) {
    	this.listModelAgentStart = newListModelAgentStart;
		this.getJListStartList().setModel(this.listModelAgentStart);
    }
    ...
    private JList<AgentClassElement4SimStart> getJListStartList() {
		if (jListStartList == null) {
			...
			MouseInputAdapter mouseHandler = new MouseInputAdapter() {
				...
				public void mouseDragged(final MouseEvent evt) {
					...
					if (toIndex != fromIndex) {
						StartSetup.this.agentMovePosition(fromIndex, toIndex);
						fromIndex = toIndex;
					}
				}
			};
			...
			// -----------------------------------------------------------------
			jListStartList = new JList<AgentClassElement4SimStart>(this.getListModelAgentStart());
			...
			jListStartList.addListSelectionListener(new ListSelectionListener() {
				...
				public void valueChanged(ListSelectionEvent e) {
					if (jListStartList.getSelectedValue() != null) {
						...
						agentSelected = jListStartList.getSelectedValue();
    ...
    /**
     * This method sets the view to the current OntologyInstanceViewer, depending on
     * the selected agent on the left
     * 
     * @param ontoInstView
     */
    ...
    private void setupLoad() {
		...
		this.setListModelAgentStart(this.currSimSetup.getAgentDefaultListModel(this.getListModelAgentStart(), SimulationSetup.AGENT_LIST_ManualConfiguration));
    ...
    /**
     * This method opens the 'AgentSelector' - dialog to select the agents for this
     * Simulation-Setup
     */
    private void agentAdd() {

		Integer startCounter = this.getListModelAgentStart().size() + 1;

		// ==================================================================
		AgentSelector agentSelector = new AgentSelector((MainWindow)Application.getMainWindow());
		...
		if (agentSelector.isCanceled() == false) {
			...
			for (int i = 0; i < agentsSelected.length; i++) {
				...
				if (this.currSimSetup.isAgentNameExists(nameSuggestion) == true) {
					ac4s.setStartAsName(this.currSimSetup.getAgentNameUnique(nameSuggestion));
				}
				// --- Add to start-list ----------------------------
				this.getListModelAgentStart().addElement(ac4s);
			}
		}
     ...
     * @param newAgentName
     */
    private void setAgentName(String newAgentName) {
		...
		// --- new name == old name ? --------------------------------
		if (this.agentSelected.getStartAsName().equalsIgnoreCase(newAgentName) == false) {
			// --- new name entered ----------------------------------
			if (this.currSimSetup.renameAgent(this.agentSelected.getStartAsName(), newAgentName)==true) {
    ...
    private void agentRemove() {
    	...
    	List<AgentClassElement4SimStart> toRemoveList = jListStartList.getSelectedValuesList();
    	for (AgentClassElement4SimStart ace4ss : toRemoveList) {
    		this.getListModelAgentStart().removeElement(ace4ss);
    	}
    ...
    /**
     * This Method changes the position / order of a selected single agent in the
     * list of the agents to start
     * 
     * @param direction
     */
    private void agentMovePosition(Integer direction) {
		...
		if (jListStartList.getSelectedValuesList().size()==1) {
			Integer position = jListStartList.getSelectedIndex();
			Integer positionNew = position + direction;
			if (positionNew >= 0 && positionNew < this.getListModelAgentStart().size()) {
				this.agentMovePosition(position, positionNew);
    ...
    /**
     * Will move an agent from an index position to an index position.
     *
     * @param fromIndexPosition the 'from' index position
     * @param toIndexPosition the 'to' index position
     */
    private void agentMovePosition(int fromIndexPosition, int toIndexPosition) {
    	
    	if (fromIndexPosition==toIndexPosition) return;
    	
    	List<ListDataListener> listenerList = Arrays.asList(this.getListModelAgentStart().getListDataListeners());
    	try {
    		// --- Temporary, remove the listener first --- 
    		listenerList.forEach(listener -> this.getListModelAgentStart().removeListDataListener(listener)); ;
    		// --- Move position --------------------------
    		AgentClassElement4SimStart ace4ss =  this.getListModelAgentStart().remove(fromIndexPosition);
    		this.getListModelAgentStart().add(toIndexPosition, ace4ss);
    		this.agentRenumberList();
    		
    	} catch (Exception ex) {
    		System.err.println("[" + this.getClass().getSimpleName() + "] Error while moving agent in start list:");
    		ex.printStackTrace();
    		
		} finally {
			// --- Re-attach the above removed listener ---
			listenerList.forEach(listener -> this.getListModelAgentStart().addListDataListener(listener)); ;
		}
    }
    ...
    private void agentRenumberList() {
		...
		for (int i = 0; i < this.getListModelAgentStart().size(); i++) {
			ac4s = this.getListModelAgentStart().getElementAt(i);
			ac4s.setPostionNo(startCounter);
			startCounter++;
		}
    }
    ...
    public void update(Observable observable, Object updateObject) {
		...
		if (updateObject instanceof SimulationSetupNotification) {
			...
			case SIMULATION_SETUP_ADD_NEW:
				this.setListModelAgentStart(new DefaultListModel<AgentClassElement4SimStart>());
				this.currSimSetup.setAgentDefaultListModel(SimulationSetup.AGENT_LIST_ManualConfiguration, this.getListModelAgentStart());
				this.setupLoad();
				break;
			default:
				this.setupLoad();
    ...
    public void actionPerformed(ActionEvent ae) {
		...
		if (trigger == jComboBoxStartLists) {
			String startListName = (String) jComboBoxStartLists.getSelectedItem();
			if (startListName!=null) {
				DefaultListModel<AgentClassElement4SimStart> dlm = this.currSimSetup.getAgentDefaultListModel(startListName);
				this.setListModelAgentStart(dlm);
				this.setJListStartListEmptySelection();
				if (startListName.equals(SimulationSetup.AGENT_LIST_ManualConfiguration)) {
			...
			this.setAgentName(jTextFieldStartAs.getText());
