...
public class SetupSelectorToolbar implements ActionListener {
	...
	private String setupAskUser4SetupName(SetupAction setupAction, String suggestion) {
		...
		while (inputIsOk==false) {
			...
			if (suggestion== null) {
				input = (String) JOptionPane.showInputDialog((MainWindow)Application.getMainWindow(), msg, head, JOptionPane.OK_CANCEL_OPTION);	
			} else {
				input = (String) JOptionPane.showInputDialog((MainWindow)Application.getMainWindow(), msg, head, JOptionPane.OK_CANCEL_OPTION, null, null, suggestion);	
			}
			...
			if (newFileName.length() < 8) {
				...
				JOptionPane.showMessageDialog((MainWindow)Application.getMainWindow(), msg, head, JOptionPane.NO_OPTION);
			} else {
				inputIsOk = true;
			}
	...
	private void setupRemove() {
		...
		int input = JOptionPane.showConfirmDialog((MainWindow)Application.getMainWindow(), msg, head, JOptionPane.YES_NO_OPTION);
