	...
	public void boot(Profile p) throws ServiceException {
		...
		if (myMainContainer==null) {
			...
			if (consoleFolder==null && Application.isOperatingHeadless()==false) {
				...
				AwbConsoleDialog consoleDialog = AgentWorkbenchUiManager.getInstance().getConsoleDialog();
				...
				AwbConsole localConsole = AgentWorkbenchUiManager.getInstance().getConsole(true);
	...
	private class ServiceComponent implements Service.Slice {
		...
		private void addConsoleLines(String containerName, Vector<String> lines2transfer) {
			...
			if (currConsole==null) {
				...
				currConsole = AgentWorkbenchUiManager.getInstance().getConsole(false);
