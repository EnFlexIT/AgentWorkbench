...
public class PathHandling {
	 ...
	 */
	public static String getFileDictionary(boolean defaultBase64, boolean absolute ){
		
		// --- TXT-Version or Base64-encoded dictionary ---
		String fileName = null;
		if (defaultBase64==true) {
			fileName = getPropertiesPath(absolute).resolve(localFileDictionary + ".bin").toString();
		} else {
			fileName = getPropertiesPath(absolute).resolve(localFileDictionary + ".csv").toString();
		}
		return fileName;
	}
