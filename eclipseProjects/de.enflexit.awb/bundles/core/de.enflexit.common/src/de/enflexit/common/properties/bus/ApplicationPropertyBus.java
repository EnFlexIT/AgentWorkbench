...
/**
 * The Class ApplicationPropertyBus enables to exchange property settings for
 * and from different parts of the current application area. Regardless, if you
 * want to update database settings or if you want to change the AWB start
 * options, the settings can be packed within a performative- or topic-driven
 * {@link Properties} instance.
 * 
 * To enable a specific information exchange, one can define and register an
 * individual {@link PropertyBusService} that is intended to cover the necessary
 * actions to set or get the context dependent setting information.
 * 
...
public class ApplicationPropertyBus {
	...
	public void removePropertyBusService(PropertyBusService perfService) {
		...
		if (perfServiceList == null)
			return;

		// --- Remove from list ---------------------------
		if (perfServiceList.remove(perfService) == false)
			return;
	...
	/**
	 * Returns all property bus services that were registered for the specified
	 * performative.
	 *
	...
	public List<PropertyBusService> getPropertyBusServices(String performative) {
		...
		if (performative == null || performative.isBlank() == true)
			return null;
	...
	public boolean setProperties(String performative, Properties properties) {
		...
		String argumentFreePerformative = performative;
		String arguments = this.getArgument(argumentFreePerformative);
		if (arguments != null) {
			argumentFreePerformative = this.getPerformativeWithoutArgument(argumentFreePerformative);
		}

		List<PropertyBusService> perfServiceListKnown = this.getPropertyBusServices(argumentFreePerformative);
		if (perfServiceListKnown != null) {
			boolean success = true;
			for (PropertyBusService pbs : perfServiceListKnown) {
				success = success & pbs.setProperties(properties, arguments);
			}
			return success;
		}
		return false;
	...
	public Properties getProperties(String performative) {
		...
		if (perfServiceListKnown != null) {
			for (PropertyBusService pbs : perfServiceListKnown) {
				properties = pbs.getProperties(properties, arguments);
			}
		}
		return properties;
	}
	...
	/**
	 * Returns the argument out of the specified performative.
	 *
	 * @param performative the performative
	 * @return the argument
	 */
	private String getArgument(String performative) {

		int argStart = performative.indexOf("[");
		int argEnd = performative.indexOf("]");
		if (argStart >= 0 && argEnd > argStart) {
			return performative.substring(argStart + 1, argEnd);
		}
		return null;
	}
	 ...
	 *
	 * @param performative the performative
	 * @return the performative without argument
	 */
	private String getPerformativeWithoutArgument(String performative) {
		...
		if (performative.contains("[") && performative.contains("]")) {
			int argStart = performative.indexOf("[");
			int argEnd = performative.indexOf("]");
			// --- Check if the argument makes sense --------------------------
			if (argStart >= 0 && argEnd > argStart) {
				return performative.substring(0, argStart);
			}
		}
		return performative;
	}
