...
public class ProgressMonitor implements ActionListener, AwbProgressMonitor, IProgressMonitor {
	 ...
	 * @param projectDesktop the parent desktop pane - may be <code>null</code>
	 * @param lookAndFeelClassName the look and feel class reference - may be <code>null</code>
	 */
	public ProgressMonitor(String windowTitle, String headerText, String progressText, ImageIcon iconImage, AwbMainWindowProjectDesktop projectDesktop, String lookAndFeelClassName) {
		...
		this.parentDesktopPane = projectDesktop;
	...
	public void setVisible(boolean visible) {
		...
		if (this.getProgressMonitorContainer() instanceof JDialog) {
			...
			if (visible==true) {
				if (this.parentDesktopPane instanceof JDesktopPane) {
					JDesktopPane jDesktopPane = (JDesktopPane) this.parentDesktopPane;
					// --- Is that a reset visible action? --------------
					boolean isResetVisible = false;
					JInternalFrame[] iFramesOpen = jDesktopPane.getAllFrames();
					...
					if (isResetVisible==false) {
						jDesktopPane.add(internalFrame, JDesktopPane.POPUP_LAYER);
						// --- Center dialog ------------				
						int top = (jDesktopPane.getHeight() - internalFrame.getHeight()) / 2; 
						int left = (jDesktopPane.getWidth() - internalFrame.getWidth()) / 2; 
