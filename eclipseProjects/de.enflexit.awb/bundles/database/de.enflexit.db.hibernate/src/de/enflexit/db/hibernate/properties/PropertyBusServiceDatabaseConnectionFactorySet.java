 ...
 */
public class PropertyBusServiceDatabaseConnectionFactorySet implements PropertyBusService {
	...
	*/
	...
	public boolean setProperties(Properties properties, String arguments) {
		...
		String factory = properties.getStringValue(FACTORYID);
		...
		// --- Prepare the hibernate database settings to be saved ------------------------------------------
		...
		// --- Save new settings and return the result ------------------------------------------------------
		return DatabaseConnectionManager.getInstance().saveDatabaseConfigurationProperties(factory, hibernateDbs);
	}
	...
	private boolean hasValidProperties(Properties properties2check) {
		...
		String factory = properties2check.getStringValue(FACTORYID);
		if (factory == null || factory.isBlank() == true) {
			invalidValues.add("Factory id is missing");
		}
