...
/**
 * The Class DerbySettingsPanel enables to configure the 
 * Derby connection in a visual way.
 * 
 * @author Christian Derksen - DAWIS - ICB - University of Duisburg-Essen
 */
public class EmbeddedDerbySettingsPanel extends AbstractDatabaseSettingsPanel {
	 ...
	 */
	public EmbeddedDerbySettingsPanel() {
		this.initialize();
	}
	/**
	...
	private void initialize() {
		...
		gbc_jLabelURLParameter.gridy = 1;
		...
		gbc_jTextFieldAddUrlParams.gridy = 1;
	...
	public void setHibernateConfigurationProperties(Properties properties) {
		...
		if (properties==null) {
			...
			properties = new DerbyDatabaseService().getHibernateDefaultPropertySettings();
		}
		String urlProperty = properties.getProperty(DerbyDatabaseService.HIBERNATE_PROPERTY_URL, "");

		// --- Remove the prefix of the URL ---------------
		String searchString = "//";
		int cutAt = urlProperty.indexOf(searchString);
		if (cutAt!=-1) urlProperty = urlProperty.substring(cutAt + searchString.length());
		...
		this.getJTextFieldDatabase().setText(properties.getProperty(DerbyDatabaseService.HIBERNATE_PROPERTY_Catalog));
		this.getJTextFieldUserName().setText(properties.getProperty(DerbyDatabaseService.HIBERNATE_PROPERTY_UserName));
		this.getJTextFieldPassword().setText(properties.getProperty(DerbyDatabaseService.HIBERNATE_PROPERTY_Password));
	...
	public Properties getHibernateConfigurationProperties() {
		...
		props.setProperty(DerbyDatabaseService.HIBERNATE_PROPERTY_DriverClass,  EmbeddedDerbyDatabaseService.getDatabaseDriverClassName()); 
		props.setProperty(DerbyDatabaseService.HIBERNATE_PROPERTY_URL, this.getUrlConfigured());
		props.setProperty(DerbyDatabaseService.HIBERNATE_PROPERTY_Catalog, this.getJTextFieldDatabase().getText());
		props.setProperty(DerbyDatabaseService.HIBERNATE_PROPERTY_UserName, this.getJTextFieldUserName().getText());
		props.setProperty(DerbyDatabaseService.HIBERNATE_PROPERTY_Password, new String(this.getJTextFieldPassword().getPassword()));
		return props;
	}
	...
	private String getUrlConfigured() {
		...
		String url = "jdbc:derby:[Database][additionalParams]";
